const en = {
  notfound: {
    primeira_linha: "Page not found",
    segunda_linha:
      "The page you tried to access does not exist or the profile provided is invalid.",
    btn_voltar: "Back to homepage",
  },
  header: {
    mnu_primeiro: "How It Works",
    mnu_segundo: "About Us",
    mnu_terceiro: "Get Started",
    mnu_planos: "Plans",
  },
  pages_home: {
    titulo_funciona: "How It Works",
    titulo_sobre: "About Us",
    titulo_planos: "Our Pricing Plans",
  },
  home: {
    titulo: "The best opportunities",
    subtitulo: "for the best people and companies",
    pefil_empresa: "Company",
    pefil_empresa_texto:
      "Top specialists and the right professionals for your company.",
    pefil_candidato: "Candidate",
    pefil_candidato_texto: "Access to opportunities that match your profile.",
    pefil_especialista: "Specialist",
    pefil_especialista_texto:
      "Visibility as a professional for growing companies.",
  },
  login: {
    mnu_entrar: "Sign In",
    mnu_criar: "Create Account",
    email: "Email",
    placehold_email: "Enter your email",
    senha: "Password",
    placehold_senha: "Enter your password",
    btn_entrar: "Sign In",
    link_esqueceu_senha: "Forgot your password?",
    carregando: "Loading...",
    erro_senha: "Invalid credentials",
    erro_campos: "Email and password are required.",
  },
  esqueceu: {
    btn_reenviar: "Resend Password",
    sucesso: "If the email exists, a reset link will be sent to it.",
  },
  cadastro: {
    nome: "First Name",
    placehold_nome: "Enter first name",
    sobrenome: "Last Name",
    placehold_sobrenome: "Enter last name",
    placehold_senha: "Create a password",
    repete_senha: "Repeat Password",
    placehold_repete_senha: "Repeat your password",
    erro_repete_senha: "Passwords do not match.",
    btn: "Register Account",
    sucesso:
      "Registration successful. Please check your email to activate your account.",
    senha_invalida:
      "Password must be between 8 and 16 characters, and include an uppercase letter, lowercase letter, number, and special character.",
    data_nascimento: "Date of Birth",
    placehold_data_nascimento: "Date of birth",
    sexo: "Gender",
    placehold_sexo: "Select your gender",
    nome_social: "Preferred Name",
    placehold_nome_social: "Enter your preferred name (optional)",
    tooltip_msg_nome_social:
      "If you provide a Social Name, it will be respected and used as your primary name on the platform.",
    item_alerta_erro_buscar_dados: "Error fetching data",
    estado: "State",
    placehold_estado: "Select the State",
    cidade: "City",
    placehold_cidade: "Select the City",
    data_invalida: "Please enter a valid date",
  },
  redefinir: {
    senha_erro: "Password must be at least 6 characters.",
    token_invalido: "Invalid or missing token.",
    erro_geral: "Error resetting password. Please try again.",
    nova_senha: "New Password",
    nova_senha_placehold: "Enter your new password",
    btn_redefinir: "Reset Password",
    redefinir_carregando: "Resetting...",
  },
  confirma_email: {
    token_invalido: "Invalid or missing token.",
    erro_geral: "Error activating the account. Invalid or expired token.",
    reenvio_link_sucesso: "Link successfully resent. Check your email.",
    reenvio_link_erro: "Error resending the link. Please try again.",
    titulo: "Email Confirmation",
    confirmando: "Confirming your email...",
    redirecionando: "Redirecting to login...",
    reenviar: "Resending...",
    btn_reenviar: "Resend activation link",
    info: "To resend the link, go to the login page.",
  },
  confirma_email_avaliador: {
    token_invalido: "Invalid or missing token.",
    erro_geral: "Error activating the account. Invalid or expired token.",
    reenvio_link_sucesso: "Link successfully resent. Check your email.",
    reenvio_link_erro: "Error resending the link. Please try again.",
    titulo: "Registration Confirmation",
    titulo_reject: "Registration Cancellation",
    confirmando: "Confirming your registration...",
    redirecionando: "Redirecting to login...",
    reenviar: "Resending...",
    btn_reenviar: "Resend activation link",
    info: "To resend the link, go to the login page.",
  },
  perfil: {
    candidato: "Candidate",
    recrutador: "Recruiter",
    avaliador: "Evaluator",
    titulo_primeiro: "Hello, welcome to whizzat!",
    titulo_segundo: "To get started, choose a profile option",
    titulo_terceiro:
      "But don't worry, you can create another profile after accessing Whizzat",
    card_candidato: "I’m a Candidate",
    card_candidato_descricao:
      "I’m looking for the best professional opportunity",
    card_recrutador: "I’m a Recruiter",
    card_recrutador_descricao:
      "I’m looking for the best candidate for a position",
    card_avaliador: "I’m an Evaluator",
    card_avaliador_descricao:
      "I want to evaluate candidates in exchange for benefits",
    btn_sair: "Logout",
    selecionado_ola: "Hello, ",
    selecionado_candidato_titulo: "Hello, Candidate",
    selecionado_candidato_descricao_curta: "I’m a Candidate",
    selecionado_candidato_cor: "green",
    selecionado_candidato_cor_css: "green",
    selecionado_recrutador_titulo: "Hello, Recruiter",
    selecionado_recrutador_descricao_curta: "I’m a Recruiter",
    selecionado_recrutador_cor: "blue",
    selecionado_recrutador_cor_css: "blue",
    selecionado_avaliador_titulo: "Hello, Evaluator",
    selecionado_avaliador_descricao_curta: "I’m an Evaluator",
    selecionado_avaliador_cor: "purple",
    selecionado_avaliador_cor_css: "purple",
    selecionado_titulo_primeiro:
      "We will only ask for information that is necessary.",
    selecionado_titulo_segundo:
      "Our policy is to ask only what is strictly necessary, so don’t worry.",
    selecionado_titulo_terceiro:
      "As you interact with Whizzat, we’ll ask for more info — but only when it’s essential.",
    selecionado_titulo_quarto:
      "On Whizzat, user profiles are identified by colors.",
    selecionado_titulo_quinto: "For the",
    selecionado_titulo_sexto: "profile, the dominant color is",
    btn_voltar: "Back",
    btn_pular_tutorial: "Skip tutorial",
    btn_avancar: "Next",
    selecionado_invalido: "Invalid profile",
  },
  semdados: {
    empresa_linha_primeiro:
      "Hello! Your company doesn’t have a registered page yet.",
    empresa_linha_segundo:
      "Complete your company information to create job postings and receive suggestions for compatible candidates.",
    empresa_linha_terceiro:
      "You can register as many companies as you like. Click the button below to get started.",
    empresa_linha_botao: "Fill in company details",
    vaga_linha_primeiro: "No job postings registered yet.",
    vaga_linha_segundo:
      "Create your first opportunity by clicking the button below.",
    vaga_linha_botao: "Register job",
    perfil_linha_primeiro: "Hello! Your ",
    perfil_linha_primeiro_complemento: "profile is not yet complete or active.",
    perfil_linha_segundo:
      "Activate or complete your profile data to use your account’s features.",
    perfil_linha_botao: "Check my profile",
    candidato_ignorado_linha_primeiro: "No ignored candidates",
    candidato_ignorado_linha_segundo:
      "Candidates you choose not to show in searches will appear here.",
  },
  sidebar: {
    menu_dashboard: "Dashboard",
    menu_avaliacoes: "My Evaluations",
    menu_convite: "Invitation / Evaluate",
    menu_meu_perfil: "My Profile",
    menu_agenda: "Schedule",
    menu_notificacoes: "Notifications",
    menu_vagas_abertas: "Open Jobs",
    menu_empresas: "Companies",
    menu_avaliadores: "Evaluators",
    menu_minhas_vagas: "My Jobs",
    menu_buscar_vagas: "Search Jobs",
    menu_sair: "Logout",
    menu_questionario: "Questionnaires",
    menu_candidatos_ignorados: "Ignored candidates",
    menu_buscar_candidatos: "Search candidates",
    minhas_oportunidades: "My opportunities",
    menu_processos: "Processes",
  },
  tela_topo_passos: {
    passo_dados: "Information",
    passo_formacao: "Education",
    passo_certificacao: "Certification",
    passo_hardskills: "Hard-Skills",
    passo_softskills: "Soft-Skills",
    passo_visualizar: "Preview",
    passo_publicar: "Publish",
    passo_localizacao: "Location",
    passo_apresentacao: "Introduction",
  },
  tela_perfil_avaliador: {
    item_ativo: "Active",
    item_inativo: "Inactive",
    item_label_situacao_cadastro: "Registration Status",
    item_label_enviando: "Sending...",
    item_msg_situacao_cadastro_aguardando: "Awaiting confirmation",
    item_msg_situacao_cadastro_confirmado: "Approved",
    item_msg_situacao_cadastro_rejeitado: "Registration Rejected",
    item_botao_situacao_cadastro: "Resend Request",
    item_label_nome: "Name",
    item_placeholder_nome: "Name",
    item_label_empresa_whizzat:
      "Do you work for a company registered on Whizzat?",
    item_msg_empresa_whizzat_sim: "Yes",
    item_msg_empresa_whizzat_nao: "No",
    item_label_empresa: "Company:",
    item_msg_empresa: "Select a company:",
    item_msg_sem_empresa: "No company",
    item_label_avaliar_vagas:
      "You wish to evaluate candidates and job openings:",
    item_msg_avaliar_vagas_todas: "From any company",
    item_msg_avaliar_todas: "Evaluate all",
    item_msg_avaliar_vagas_minha: "Only from my company",
    item_msg_avaliar_minha: "Evaluate only from my company",
    item_label_telefone: "Contact phone:",
    item_placeholder_telefone: "Phone number",
    item_label_localizacao: "Location:",
    item_placeholder_localizacao: "Enter your location",
    item_label_notificacao: "Notification method:",
    item_msg_notificacao_whatsapp: "WhatsApp",
    item_msg_notificacao_sms: "SMS",
    item_label_descreva:
      "Describe yourself in a few lines. This text will serve as your introduction letter to companies on the platform.",
    item_label_descreva_negrito:
      "(If you belong to a single company, you may leave this field blank.)",
    item_placeholder_apresentacao: "Introduce yourself",
    item_msg_nenhuma_apresentacao: "No introduction provided.",
    item_label_foto:
      "Upload a photo, image, or logo (recommended 512×512 px, up to 1MB):",
    item_msg_foto: "Click here to upload your image",
    item_msg_sem_foto: "No logo",
    item_label_informe_formacao: "Enter your academic background",
    item_label_graduacao: "Degree:",
    item_msg_graduacao: "Select your degree",
    item_label_formacao: "Field of Study:",
    item_msg_formacao: "Enter your field of study",
    item_label_certificado: "Certificate (JPG, PNG, or PDF):",
    item_msg_certificado: "Select a file",
    item_botao_adicionar: "+ Add",
    item_botao_visualizar: "View Certificate",
    item_botao_substituir: "Replace Certificate",
    item_msg_nenhuma_graduacao: "No degree found",
    item_botao_remover_formacao: "Remove Education",
    item_label_informe_certificacao: "Enter your certifications",
    item_label_certificacao: "Certifications:",
    item_msg_certificacao: "Type or select",
    item_msg_criar_certificacao: "Create new certification:",
    item_botao_remover_certificacao: "Remove Certification",
    item_botao_certificado_atualizado: "Certificate updated!",
    item_label_informe_hardskills:
      "Enter the Hard Skills you wish to work with as an evaluator",
    item_label_informe_hardskills_subitem:
      "Hard Skills are the technical competencies acquired by professionals throughout their careers.",
    item_label_informe_softskills:
      "Enter the Soft Skills you wish to work with as an evaluator",
    item_label_informe_softskills_subitem:
      "Soft Skills are the behavioral competencies that are natural or acquired by professionals throughout their careers.",
    item_label_informe_qtde: "(minimum 3 and maximum 12)",
    item_label_skill: "Skills:",
    item_msg_skill: "Type or select a skill",
    item_msg_criar_skill: "Create new skill:",
    item_label_peso: "Level",
    item_label_favorito: "I want to be an evaluator:",
    item_placeholder_favorito: "Skill duration",
    item_botao_remover_skill: "Remove Skill",
    item_label_lista_formacao: "Education",
    item_msg_nenhuma_formacao: "No education records found",
    item_label_lista_certificacao: "Certifications",
    item_msg_nenhuma_certificacao: "No certifications found",
    item_botao_voltar: "Back",
    item_botao_cancelar: "Cancel",
    item_botao_avancar: "Next",
    item_botao_publicar: "Publish",
    item_msg_campo_obt: "Required field",
    item_msg_logo_obt: "Logo required",
    item_alerta_descartada: "Changes discarded.",
    item_alerta_sucesso: "Evaluator published successfully!",
    item_alerta_erro_buscar_dados: "Error fetching evaluator data.",
    item_alerta_erro_tipo_arq:
      "Only image files are allowed (JPG, PNG, SVG, etc.)",
    item_alerta_erro_tamanho_arq: "The file must be no larger than 1MB.",
    item_alerta_erro_tamanho_arq2: "The file must be no larger than 2MB.",
    item_alerta_erro_salvar: "Error saving evaluator.",
    item_alerta_erro_reenviar_solicitacao: "Error resending request.",
    item_alerta_reenviar_sucesso: "Request resent successfully!",
    item_tooltip_certificacao_titulo:
      "How to add a certification that is not on the list:",
    item_tooltip_certificacao_passo1: "1. Type the desired certification;",
    item_tooltip_certificacao_passo2: "2. Select 'Create new certification';",
    item_tooltip_certificacao_passo3: "3. Click the Add button.",
    item_tooltip_skill_titulo: "How to add a skill that is not on the list:",
    item_tooltip_skill_passo1: "1. Type the desired skill;",
    item_tooltip_skill_passo2: "2. Select 'Create new skill';",
    item_tooltip_skill_passo3: "3. Click the Add button.",
    item_tooltip_favorite_passo1:
      "- You can select up to 6 skills to be an evaluator.",
    item_tooltip_favorite_passo2: "- The selected skills will be considered ",
    item_tooltip_favorite_passo2_1: "as preferred for candidate evaluations.",
  },
  tela_perfil_recrutador: {
    item_ativo: "Active",
    item_inativo: "Inactive",
    item_label_nome: "Name",
    item_placeholder_primeiro_nome: "First Name",
    item_placeholder_ultimo_nome: "Last Name",
    item_label_telefone: "Contact phone:",
    item_placeholder_telefone: "Phone",
    item_label_localizacao: "Location:",
    item_placeholder_localizacao: "Enter your location",
    item_label_notificacao: "Notification method:",
    item_msg_notificacao_whatsapp: "WhatsApp",
    item_msg_notificacao_sms: "SMS",
    item_label_descreva:
      "Describe, in a few lines, a bit more about yourself. This text will be your introduction letter for companies on the platform.",
    item_label_descreva_negrito:
      "(If you belong to only one company, you can leave this field blank)",
    item_placeholder_apresentacao: "Introduce yourself",
    item_msg_nenhuma_apresentacao: "No introduction provided.",
    item_label_foto:
      "Upload a photo, image, or recruiter logo (recommended 512×512 px, up to 1MB):",
    item_msg_foto: "Click here to upload your image",
    item_msg_sem_foto: "No logo",
    item_botao_voltar: "Back",
    item_botao_cancelar: "Cancel",
    item_botao_avancar: "Next",
    item_botao_publicar: "Publish",
    item_msg_campo_obt: "Required field",
    item_msg_logo_obt: "Logo required",
    item_alerta_descartada: "Changes discarded.",
    item_alerta_sucesso: "Recruiter successfully published!",
    item_alerta_erro_buscar_dados: "Error fetching recruiter data",
    item_alerta_erro_tipo_arq: "Upload image files only (JPG, PNG, SVG, etc.)",
    item_alerta_erro_tamanho_arq: "File must be at most 1MB.",
    item_alerta_erro_salvar: "Error saving recruiter.",
  },
  tela_perfil_candidato: {
    item_ativo: "Active",
    item_inativo: "Inactive",
    item_label_nome: "Name",
    item_placeholder_nome: "Name",
    item_label_telefone: "Contact Phone:",
    item_placeholder_telefone: "Phone",
    item_label_localizacao: "Location:",
    item_placeholder_localizacao: "Enter your location",
    item_label_notificacao: "Notification Method:",
    item_msg_notificacao_whatsapp: "WhatsApp",
    item_msg_notificacao_sms: "SMS",

    item_label_descreva:
      "Describe yourself in a few lines. This text will be your cover letter for companies on the platform.",
    item_placeholder_apresentacao: "Introduce yourself",
    item_msg_nenhuma_apresentacao: "No introduction provided.",

    item_label_foto:
      "Upload a candidate photo, image, or logo (recommended 512×512 px, up to 1MB):",
    item_msg_foto: "Click here to upload your image",
    item_msg_sem_foto: "No image",

    item_label_informe_formacao: "Enter your academic background",
    item_label_graduacao: "Degree:",
    item_msg_graduacao: "Select a degree",
    item_label_formacao: "Education:",
    item_msg_formacao: "Enter your education",
    item_label_certificado: "Certificate (JPG, PNG, or PDF):",
    item_msg_certificado: "Select the file",
    item_botao_adicionar: "+ Add",
    item_botao_visualizar: "View Certificate",
    item_botao_substituir: "Replace Certificate",
    item_msg_nenhuma_graduacao: "No degree found",
    item_botao_remover_formacao: "Remove Education",

    item_label_informe_certificacao: "Enter your certifications",
    item_label_certificacao: "Certifications:",
    item_msg_certificacao: "Type or select",
    item_msg_criar_certificacao: "Create new certification:",
    item_botao_remover_certificacao: "Remove Certification",
    item_botao_certificado_atualizado: "Certificate Updated!",

    item_label_informe_hardskills:
      "Enter the Hard Skills you want to work with as a candidate",
    item_label_informe_hardskills_subitem:
      "Hard Skills are the technical competencies acquired by professionals throughout their careers.",
    item_label_informe_softskills:
      "Enter the Soft Skills you want to work with as a candidate",
    item_label_informe_softskills_subitem:
      "Soft Skills are the behavioral competencies that are natural or acquired throughout a professional’s career.",

    item_label_informe_qtde: "(minimum 3 and maximum 12)",
    item_label_skill: "Skills:",
    item_msg_skill: "Type or select a skill",
    item_msg_criar_skill: "Create new skill:",
    item_label_peso: "Level",
    item_label_favorito: "Favorite",
    item_placeholder_favorito: "Skill time",
    item_botao_remover_skill: "Remove Skill",

    item_label_lista_formacao: "Education",
    item_msg_nenhuma_formacao: "No education added",
    item_label_lista_certificacao: "Certifications",
    item_msg_nenhuma_certificacao: "No certification added",

    item_botao_voltar: "Back",
    item_botao_cancelar: "Cancel",
    item_botao_avancar: "Next",
    item_botao_publicar: "Publish",

    item_msg_campo_obt: "Required Field",
    item_msg_logo_obt: "Image Required",

    item_alerta_descartada: "Changes discarded.",
    item_alerta_sucesso: "Candidate published successfully!",
    item_alerta_erro_buscar_dados: "Error fetching candidate data",
    item_alerta_erro_tipo_arq: "Upload only image files (JPG, PNG, SVG, etc.)",
    item_alerta_erro_tamanho_arq: "File must be up to 1MB.",
    item_alerta_erro_tamanho_arq2: "File must be up to 2MB.",
    item_alerta_erro_salvar: "Error saving candidate.",

    item_tooltip_certificacao_titulo:
      "How to add a certification that is not in the list:",
    item_tooltip_certificacao_passo1: "1. Type the desired certification;",
    item_tooltip_certificacao_passo2: "2. Select 'Create new certification';",
    item_tooltip_certificacao_passo3: "3. Click the Add button.",

    item_tooltip_skill_titulo: "How to add a skill that is not in the list:",
    item_tooltip_skill_passo1: "1. Type the desired skill;",
    item_tooltip_skill_passo2: "2. Select 'Create new skill';",
    item_tooltip_skill_passo3: "3. Click the Add button.",

    item_tooltip_favorite_passo1: "- You may select up to 6 favorites.",
    item_tooltip_favorite_passo2: "- Favorite skills will be used ",
    item_tooltip_favorite_passo2_1: " for candidate evaluations.",
    item_sugerido_avaliador: "Recommended by the evaluator:",
    aberto_oportunidades: "Open to Opportunities",

    disponivel_oportunidades: "Available for New Opportunities",

    indisponivel_oportunidades: "Not Available at the Moment",

    inclusao_titulo: "Inclusive and Affirmative Action Opportunities",

    inclusao_descricao:
      "These options are voluntary and help identify inclusive or affirmative action opportunities that match your profile.",

    inclusao_pcd: "Opportunities for People with Disabilities (PWD)",

    inclusao_pcd_msg:
      "I would like to participate in opportunities intended for people with disabilities.",

    inclusao_racial: "Ethnic and Racial Affirmative Action",

    inclusao_racial_msg:
      "I would like to participate in opportunities with ethnic and racial affirmative action policies.",

    inclusao_lgbtqia: "LGBTQIA+ Diversity",

    inclusao_lgbtqia_msg:
      "I would like to participate in opportunities focused on LGBTQIA+ diversity.",

    inclusao_50mais: "Professionals 50+",

    inclusao_50mais_msg:
      "I would like to participate in opportunities for professionals aged 50 and over.",

    inclusao_diversidade: "Other Diversity and Inclusion Initiatives",

    inclusao_diversidade_msg:
      "I would like to be considered for other inclusive opportunities that match my profile.",

    inclusao_privacidade:
      "Providing this information is optional. These preferences will only be used to identify inclusive or affirmative action opportunities and will not affect your participation in opportunities open to all candidates.",

    inclusao_visualizacao_msg:
      "Preferences provided for participation in inclusive or affirmative action opportunities.",
    inclusao_pcd_curto: "PWD",
    inclusao_racial_curto: "Ethnic and Racial Affirmative Action",
    inclusao_lgbtqia_curto: "LGBTQIA+ Diversity",
    inclusao_50mais_curto: "Professionals 50+",
    inclusao_diversidade_curto: "Diversity and Inclusion",
    inclusao_nenhuma_selecionada:
      "No inclusive opportunity preferences have been selected.",
    inclusao_privacidade_resumo:
      "This information is optional and will only be used to identify opportunities that match your preferences.",
    modalidade_titulo: "Work Arrangement",
    modalidade_descricao:
      "Select the work arrangements you are willing to consider for new opportunities.",
    modalidade_presencial: "On-site",
    modalidade_hibrido: "Hybrid",
    modalidade_remoto: "Remote",
    modalidade_visualizacao_msg:
      "Work arrangements you are willing to consider for new opportunities.",
    modalidade_nenhuma_selecionada: "No work arrangement has been selected.",
  },
  tela_lista_avaliadores: {
    item_titulo_aguardando: "Evaluators Awaiting Confirmation",
    item_titulo_reprovados: "Rejected Evaluators",
    item_titulo_aprovados: "Approved Evaluators",
    item_msg_sem_especialista: "No evaluators in this category.",
    item_msg_sem_foto: "No logo",
    item_botao_aprovar: "Approve",
    item_botao_reprovar: "Reject",
    item_alerta_sucesso: "Evaluator successfully updated!",
    item_alerta_erro_buscar_dados: "Error fetching evaluator data",
    item_alerta_erro_salvar: "Error performing operation!",
  },
  tela_lista_empresas: {
    item_ativo: "Active",
    item_inativo: "Inactive",
    item_msg_sem_foto: "No logo",
    item_botao_cadastrar: "+ Add Company",
    item_alerta_erro_buscar_dados: "Error fetching companies",
    item_msg_sem_email: "No email",
    item_msg_sem_telefone: "No phone",
  },
  tela_empresa_dados: {
    item_ativo: "Active",
    item_inativo: "Inactive",
    item_label_nome: "Company Name",
    item_placeholder_nome: "Company",
    item_label_site: "Company Website",
    item_placeholder_site: "Website",
    item_label_email: "Contact Email",
    item_placeholder_email: "Email",
    item_label_telefone: "Contact Phone:",
    item_placeholder_telefone: "Phone",
    item_label_titulo_localizacao:
      "Enter your location to find the best candidates",
    item_label_localizacao: "Location:",
    item_placeholder_localizacao: "Enter your company's location",
    item_label_titulo_descreva: "Company Overview",
    item_label_descreva:
      "Describe in a few lines who your company is and what it seeks professionally. This text will serve as your introduction to candidates and specialists on the platform:",

    item_placeholder_apresentacao: "Introduce your company",
    item_msg_nenhuma_apresentacao: "No description provided.",
    item_label_foto:
      "Upload your company's logo (recommended 512×512 px, up to 1MB):",
    item_msg_foto: "Click here to upload your image",
    item_label_capa:
      "Upload your company's cover image (recommended: 1200×300 px, up to 1MB):",
    item_msg_sem_foto: "No logo",
    item_msg_sem_capa: "No cover image",
    item_botao_voltar: "Back",
    item_botao_cancelar: "Cancel",
    item_botao_avancar: "Next",
    item_botao_publicar: "Publish",
    item_msg_campo_obt: "Required Field",
    item_msg_logo_obt: "Logo Required",
    item_msg_capa_obt: "Cover Required",
    item_alerta_descartada: "Changes discarded.",
    item_alerta_sucesso_1: "Company ",
    item_alerta_sucesso_2: "successfully published!",
    item_alerta_erro_buscar_dados: "Error fetching company data",
    item_alerta_erro_tipo_arq:
      "Please upload only image files (JPG, PNG, SVG, etc.)",
    item_alerta_erro_tamanho_arq: "The file must be a maximum of 1MB.",
    item_alerta_erro_salvar: "Error saving company.",
    item_botao_lista_empresa: "← Back to company list",
    item_botao_cadastrar: "+ Add Company",
    item_botao_editar: "Edit Company",
    item_botao_vaga: "+ Add Job",
    item_item_vagas: "Jobs",
    item_item_periodo: "Open until:",
  },
  tela_lista_vagas: {
    item_titulo_sugeridos: "Suggested candidates: ",
    item_titulo_escolhidos: "Selected candidates:",
    item_titulo_avaliacao: "Candidates under evaluation:",
    item_msg_empresas: "All companies",
    item_msg_skills: "All skills",
    item_msg_modalidades: "All work modalities",
    item_item_periodo: "Open until:",
    item_msg_sem_foto: "No logo",
    item_botao_filtrar: "Filter",
    item_botao_cadastrar: "+ Register Jobs",
    item_alerta_erro_buscar_dados: "Error fetching jobs",
    item_msg_sem_vagas: "No job openings available with the selected criteria.",
    item_msg_sem_vagas_filtro:
      "Try adjusting the filters or clearing the selection to see more results.",
    item_msg_item_mulher: "Women",
  },
  tela_vaga_dados: {
    item_ativo: "Active",
    item_inativo: "Inactive",
    item_situacao: "Status:",
    item_label_nome: "Company:",
    item_placeholder_nome: "Select a company",
    item_msg_indefinida: "Undefined",
    item_label_site: "Job title:",
    item_placeholder_site: "Job title",
    item_label_local: "Job location:",
    item_placeholder_local: "Job location",
    item_msg_sem_local: "Location not provided",
    item_label_descricao: "Description:",
    item_placeholder_descricao: "Job description",
    item_label_modalidade: "Work mode:",
    item_label_hibrido: "Hybrid",
    item_label_home: "Remote",
    item_label_presenial: "On-site",
    item_msg_sem_modalidade: "Mode not provided",
    item_label_periodo: "Work schedule:",
    item_label_integral: "Full-time",
    item_label_meio: "Part-time",
    item_label_hora: "Hourly",
    item_msg_sem_periodo: "Schedule not provided",
    item_label_dias_vaga: "How many days will the position remain open?",
    item_label_qtde_vaga: "Number of openings for this position:",
    item_label_vaga_afirmativa: "Affirmative / Inclusive position:",
    item_vaga_pcd: "Position for PWD",
    item_vaga_lgbtq: "Position for LGBTQ+",
    item_vaga_mulheres: "Position for Women",
    item_vaga_50_mais: "Position for 50+",
    item_msg_item_mulher: "Women",
    item_msg_sem_foto: "No logo",
    item_botao_voltar: "Back",
    item_botao_cancelar: "Cancel",
    item_botao_avancar: "Next",
    item_botao_publicar: "Publish",
    item_msg_campo_obt: "Required field",
    item_alerta_descartada: "Changes discarded.",
    item_alerta_sucesso_1: "Job ",
    item_alerta_sucesso_2: "successfully published!",
    item_alerta_erro_buscar_dados: "Error fetching job data",
    item_alerta_erro_salvar: "Error saving job.",
    item_botao_cadastrar: "+ Register Job",
    item_botao_editar: "Edit Job",
    item_botao_adicionar: "+ Add",
    item_item_vagas: "Jobs",
    item_item_periodo: "Open until:",
    item_msg_vigencia: "Valid until: ",
    item_msg_aberta: "Opened on: ",
    item_msg_sem_data: "Date not provided",
    item_label_informe_hardskills:
      "Specify the Hard Skills you are looking for in this professional profile",
    item_label_informe_hardskills_subitem:
      "Hard Skills are the technical competencies acquired by professionals throughout their careers.",
    item_label_informe_softskills:
      "Specify the Soft Skills you are looking for in this professional profile",
    item_label_informe_softskills_subitem:
      "Soft Skills are the behavioral competencies that are natural or acquired by professionals throughout their careers.",
    item_label_informe_qtde: "(minimum 3 and maximum 12)",
    item_label_skill: "Skills:",
    item_msg_skill: "Type or select a skill",
    item_msg_criar_skill: "Create new skill:",
    item_label_skill_pesos: "Skills and Level",
    item_label_peso: "Level",
    item_label_avaliador: "Evaluator",
    item_label_proprio: "Internal",
    item_label_whizzat: "Whizzat",
    item_botao_remover_skill: "Remove Skill",
    item_tooltip_skill_titulo: "How to add a skill not in the list:",
    item_tooltip_skill_passo1: "1. Type the desired skill;",
    item_tooltip_skill_passo2: "2. Select 'Create new skill';",
    item_tooltip_skill_passo3: "3. Click the Add button.",
    item_tooltip_avaliador_passo1: "1. Internal: Will be selected ",
    item_tooltip_avaliador_passo2: "only among existing evaluators ",
    item_tooltip_avaliador_passo3: "within the recruiting company.",
    item_tooltip_avaliador_passo4: "2. Whizzat: Will be selected ",
    item_tooltip_avaliador_passo5: "automatically by the platform, ",
    item_tooltip_avaliador_passo6: "considering top skills ",
    item_tooltip_avaliador_passo7: "and evaluations. ",
    item_tooltip_avaliador_passo8: "1. Whizzat: Will be selected ",
    item_tooltip_avaliador_passo9: "automatically by the platform, ",
    item_tooltip_avaliador_passo10: "considering top skills ",
    item_tooltip_avaliador_passo11: "and evaluations, and when the company ",
    item_tooltip_avaliador_passo12: "selected does not have its own ",
    item_tooltip_avaliador_passo13: "internal evaluator.",
    item_botao_lista_vagas: "← Back to job list",
    item_label_panel_score: "Average Score  (Hard+Soft):",
    item_label_panel_hardskill: "Hard Skill Distribution",
    item_label_panel_softskill: "Soft Skill Distribution",
    tipo_grafico_perfil: "Profile",
    tipo_grafico_avaliador: "Evaluator Recommendation",
    tipo_oportunidade_titulo: "Opportunity Type",
    tipo_oportunidade_descricao:
      "Define whether the position is open to all candidates or has a specific affirmative action policy.",
    tipo_ampla: "Open Competition",
    tipo_ampla_msg: "Opportunity open to all qualified candidates.",
    tipo_afirmativa: "Affirmative Action",
    tipo_afirmativa_msg:
      "Opportunity focused on or prioritizing specific groups.",
    tipo_exclusiva: "Exclusive",
    tipo_exclusiva_msg:
      "Opportunity exclusively intended for the selected groups.",
    publico_afirmativo_titulo: "Target Audience",
    publico_afirmativo_msg:
      "Select the groups covered by this affirmative action policy.",
    publico_exclusivo_msg:
      "Select the groups this opportunity is intended for.",
    publico_pcd: "People with Disabilities (PWD)",
    publico_racial: "Ethnic and Racial Affirmative Action",
    publico_lgbtqia: "LGBTQIA+ Diversity",
    publico_mulheres: "Women",
    publico_50mais: "Professionals 50+",
    publico_diversidade: "Other Diversity Initiatives",
    publico_obrigatorio:
      "Select at least one target group for this opportunity.",
    tipo_oportunidade_detalhe_msg:
      "Classification and target audience for this opportunity.",
  },
  vaga_match: {
    titulo: "Find candidates",
    descricao:
      "Find candidates with the highest compatibility with this job's criteria.",
    compatibilidade: "Compatibility",
    compatibilidade_alta: "High compatibility",
    compatibilidade_boa: "Good compatibility",
    compatibilidade_compativel: "Compatible",
    compatibilidade_todos: "All",
    quantidade: "Results",
    buscar: "Find candidates",
    buscando: "Searching...",
    criterios_considerados: "Criteria automatically considered in this search:",
    criterio_skills: "Job skills",
    criterio_modalidade: "Work arrangement",
    criterio_localizacao: "Location",
    criterio_oportunidade: "Opportunity criteria",
    resultados_titulo: "Compatible candidates",
    resultados_descricao:
      "Results sorted by highest compatibility with this job.",
    candidatos_encontrados: "candidates found",
    match_geral: "Overall match",
    hard_skills: "Hard Skills",
    soft_skills: "Soft Skills",
    modalidade_compativel: "Compatible work arrangement",
    oportunidade_compativel: "Compatible opportunity",
    skills_avaliadas: "skills evaluated",
    ver_perfil: "View profile",
    selecionados: "selected",
    selecionar_todos: "Select all",
    convidar_selecionados: "Invite selected",
    mais_acoes: "More actions",
    nao_mostrar_novamente: "Don't show again",
    ignorar_titulo: "Don't show this candidate again?",
    ignorar_descricao:
      "This candidate will no longer appear in your future searches. You will be able to reverse this decision later.",
    ignorar_motivo: "Reason",
    ignorar_motivo_opcional: "Select a reason (optional)",
    motivo_perfil: "Profile not suitable",
    motivo_entrevista: "Interview history",
    motivo_avaliado: "Previously evaluated",
    motivo_outro: "Other",
    cancelar: "Cancel",
    confirmar_ignorar: "Don't show again",
    sem_resultados: "No candidates available for this search.",
    limite_convites: "You can select up to 10 candidates at a time.",
    compatibilidade_alta_descricao: "90% or higher",
    compatibilidade_boa_descricao: "75% or higher",
    compatibilidade_compativel_descricao: "60% or higher",
    compatibilidade_todos_descricao: "No minimum percentage",
    ir_para_busca: "Find candidates",
    erro_buscar: "Unable to find candidates for this job.",
    publico_prioritario: "Priority audience",
    ignorando: "Removing...",
    ignorar_sucesso: "Candidate removed from future searches.",
    erro_ignorar: "Unable to remove this candidate from future searches.",
    menu_candidatos_ignorados: "Ignored candidates",
    enviando_convites: "Sending invitations...",
    convites_enviados_sucesso: "{{total}} invitation(s) sent successfully.",
    erro_enviar_convite: "Unable to send the invitations.",
    todos_ja_convidados:
      "The selected candidates have already been invited to this position.",
    convites_enviados_parcial:
      "{{total}} invitation(s) sent. {{jaConvidados}} candidate(s) had already been invited.",
    confirmar_convite_titulo: "Send invitation?",
    confirmar_convite_descricao: "Confirm the invitation before continuing.",
    confirmar_convite_um:
      "You are about to invite 1 candidate to this position.",
    confirmar_convite_varios:
      "You are about to invite {{total}} candidates to this position.",
    confirmar_convite_aviso:
      "Once sent, candidates will be notified and will be able to accept or decline the invitation.",
    confirmar_convite_cancelar: "Cancel",
    confirmar_convite_enviar: "Send invitation",
    confirmar_convite_fechar: "Close",
    ja_convidado: "Already invited",
    ja_convidado_anteriormente: "Previously invited by you",
  },
  candidatos_ignorados: {
    titulo: "Candidatos ignorados",
    descricao: "Gerencie candidatos que você escolheu não mostrar nas buscas.",
    selecionar_todos: "Selecionar todos",
    selecionados: "selecionado(s)",
    restaurar_selecionados: "Restaurar selecionados",
    voltar_mostrar: "Voltar a mostrar",
    restaurando: "Restaurando...",
    sem_candidatos: "Você não possui candidatos ignorados.",
    sem_motivo: "Motivo não informado",
    motivo_perfil: "Perfil não aderente",
    motivo_entrevista: "Histórico de entrevista",
    motivo_avaliado: "Já avaliado anteriormente",
    motivo_outro: "Outro",
    erro_carregar: "Não foi possível carregar os candidatos ignorados.",
    erro_restaurar: "Não foi possível restaurar o candidato.",
    restaurar_sucesso: "Candidato voltou a aparecer nas suas buscas.",
    restaurar_varios_sucesso: "Candidatos restaurados com sucesso.",
  },
  planos: {
    titulo: "You don’t have a plan yet. Please select one below.",
    recomendado: "Recommended",
    botao_selecionar: "Select Plan",
    botao_selecionado: "Selected Plan",
    mensal: "Monthly",
    anual: "Yearly",
    mes: "month",
    ano: "year",
  },
  notificacao: {
    novo: "New",
    candidato: "Candidate:",
    enviado: "Sent",
    marcar: "Mark as read",
    lida: "Read",
    marcar_todos: "Mark all as read",
    filtro_todas: "All",
    filtro_nao_lidas: "Unread",
    titulo_confirma_exclusao: "Confirm deletion",
    msg_confirma_exclusao: "Do you really want to delete this notification?",
    botao_cancelar: "Cancel",
    botao_excluir: "Delete",
    msg_exclusao: "Notification successfully deleted",
    msg_erro: "Error deleting notification",
    carregando: "Loading more...",
  },
  minha_avaliacao: {
    recebido: "Received Invitations",
    aceito: "Accepted / Awaiting Questionnaire",
    agendado: "Schedule / Scheduled",
    sem_convite: "No pending invitations",
    sem_aceito: "No accepted invitations",
    sem_agenda: "No pending schedules",
    aguardando_finalizacao: "Awaiting Completion",
    aguardando_avaliacao: "Awaiting Evaluation",
    aguardando_agenda: "Awaiting Scheduling",
    aguardando_questionario: "Pending Questionnaire",
    aguardando_entrevista: "Awaiting Interview",
    questionario_respondido: "Questionnaire Submitted",
    agenda_recusada: "Date Declined",
    agenda_aceita: "Schedule Confirmed",
    btn_aceitar: "Accept",
    btn_recusar: "Decline",
    peso: "Level",
    aceito_em: "Accepted on ",
    enviado_em: "Sent on ",
    sem_finalizacao: "No completed evaluations",
    finalizado: "Completed",
    basico: "Basic",
    intermediario: "Intermediate",
    avancado: "Advanced",
    autoavaliacao: "Self-Assessment",
    ultima_avaliacao: "Last Evaluation",
    dt_ultima_avaliacao: "Last Evaluation Date",
    titulo_progresso: "Evaluation Progress",
    etapa_atual: "current stage",
    titulo_questionario: "Select Questionnaire",
    carregando_questionario: "Loading questionnaires...",
    selecione_questionario: "Select a questionnaire",
    nao_questionario: "Do Not Send Questionnaire",
    enviar_questionario: "Send Questionnaire",
    enviando: "Sending...",
    enviado_questionario: "Questionnaire sent successfully",
    questionario: "Questionnaire",
    data_envio: "Sent Date",
    data_resposta: "Response Date",
    data_entrevista: "Interview Date",
    data_sugestao: "Suggestion Sent",
    horarios: "Time Slots",
    sugestao_selecionada: "Selected Suggestion",
    nova_sugestao: "Send New Suggestion",
    enviada_sugestao: "Suggestion Sent",
    enviar_sugestao: "Send Suggestion",
    confirmar_entrevista: "Confirm Interview Completion",
    msg_agenda:
      "Available after the questionnaire is sent or if the evaluator chooses not to send one",
    avaliacao_finalizada: "Evaluation Completed!",
    erro_problema: "An unexpected error occurred. Please try again later.",
    finalizar_avaliacao: "Complete Evaluation",
    finalizada_avaliacao: "Evaluation Completed",
    informe_nota: "Enter the final score and interview feedback",
    nota_final: "Final Score",
    comentario:
      "Comment (visible only to you. This content will not be displayed to the candidate)",
    comentario_resposta: "Response Comment",
    finalizando: "Completing...",
    btn_ver_resposta: "View Answers",
    btn_ocultar_resposta: "Hide Answers",
    sem_questionario: "No questionnaire found",
    msg_salvar_comentario: "Comment saved.",
    informe_comentario: "A comment is required",
    respostas_questionario: "Questionnaire Responses",
    salvando: "Saving...",
    salvar_comentario: "Save Comment",
    comentario_finalizacao:
      "Comment (this content will be shared with the candidate as feedback. Please use clear, objective, and constructive language.)",
    steps: {
      convite_aceito: "INVITATION ACCEPTED",
      questionario_enviado: "QUESTIONNAIRE SENT",
      agenda_enviada: "SCHEDULE SENT",
      agendado: "SCHEDULED",
      entrevista_realizada: "INTERVIEW COMPLETED",
      finalizado: "COMPLETED",
    },
    status_agenda: {
      pendente: "Awaiting Acceptance",
      aceito: "Accepted",
      recusado: "Declined",
    },
    candidato: {
      questionarios_pendentes: "Pending Questionnaires",
      entrevistas: "Suggested Interviews",
      finalizadas: "Completed",
      sem_questionario: "No pending questionnaires",
      sem_entrevista: "No scheduled interviews",
      sem_finalizacao: "No completed evaluations",
      avaliacao_finalizada: "Evaluation Completed",
      atualizado_em: "Updated on: ",
      entrevista_confirmada: "Interview Confirmed",
      responder_questionario: "Awaiting Response",
      btn_recusar: "Decline",
      btn_aceitar: "Accept",
      peso: "Level:",
      entrevista_em: "Interview",
      questionario_pendente: "Awaiting Response",
      formulario_enviado: "Questionnaire Received:",
      entrevista_recusada: "Date declined. Waiting for a new proposed date.",
      resultado_avaliacao: "Evaluation Result",
      resultado_descricao:
        "Below you can review your self-assessment, the evaluator's score, and the level identified during the evaluation process.",
      basico: "Basic",
      intermediario: "Intermediate",
      avancado: "Advanced",
      comentario: "Evaluation Feedback",
      comentario_msg:
        "Below is the feedback provided by the evaluator regarding your interview.",
    },
  },
  questionario: {
    titulo_pagina: "Questionnaire Creation",
    pagina_descricao: "",
    titulo: "Questionnaire Title",
    place_titulo: "Enter a title for your questionnaire",
    comentario: "Comment",
    place_comentario:
      "Optionally add a comment describing the purpose or intended use of this questionnaire",
    ativo: "Active",
    inativo: "Inactive",
    pergunta: "Questions",
    btn_add_pergunta: "Add Question",
    sem_pergunta: "There are no questions in your questionnaire yet",
    btn_cancelar: "Cancel",
    btn_salvar: "Save",
    btn_salvando: "Saving...",
    criado_em: "Created on",
    titulo_pagina_questionario: "Questionnaire List",
    btn_add_questionario: "New Questionnaire",
    btn_ver: "View",
    btn_editar: "Edit",
    btn_excluir: "Delete",
    sem_questionario: "No questionnaires have been created yet",
    sucesso: "Request completed successfully!",
    erro: "There was a problem processing the request!",
    nao_encontrado: "Questionnaire not found",
    ver_pergunta: "Question",
    btn_voltar: "Back",
    resposta_base: "Sample Answer",
    tipo_resposta: "Answer Type",
    titulo_editar: "Edit Questionnaire",
    btn_excluindo: "Deleting...",
    filtro: "Filter",
    modal_titulo: "Delete Questionnaire",
    modal_pergunta: "Are you sure you want to delete",
    anterior: "Previous",
    proximo: "Next",
    pagina_count: "Page",
    de_count: "of",
    msg_titulo_obg: "Please enter the questionnaire title",
    msg_pergunta_obg: "Add at least one question",
    msg_todaspergunta_obg: "All questions must be completed",
    excluir_pergunta: "Delete Question",
    duplicar_pergunta: "Duplicate Question",
    msg_exclusao_pergunta:
      "If the question has already been sent to a candidate, it will only be deactivated.",
    msg_nenhuma_pergunta: "No questions have been added yet",
    ativo_sim: "Yes",
    ativo_nao: "No",
    resultado_busca: "questionnaire(s) found",
    filtro_busca: "Search questionnaires...",
    filtro_ativo_todos: "All",
    filtro_ativo: "Active",
    filtro_inativo: "Inactive",
    obrigatorio: "Required",
    resposta: {
      titulo: "Evaluation Questionnaire",
      titulo_skill: "Evaluated Skill:",
      titulo_respondida: "Answered:",
      titulo_de: "of",
      btn_cancelar: "Cancel",
      salvando: "Saving...",
      btn_salvar: "Submit Answers",
      msg_sem_questionario: "Questionnaire not found.",
      msg_carregando: "Loading questionnaire...",
      msg_erro: "Error saving questionnaire",
      msg_sucesso: "Questionnaire submitted successfully",
      msg_preencher_campo:
        "Please complete all required questions before submitting the questionnaire.",
      finalizar_sem_entrevista: "Allow Completion Without Interview",
    },
  },
  agenda: {
    titulo_um: "Interview Schedule",
    titulo_dois: "View your upcoming scheduled interviews below.",
    status_aguardando: "Awaiting Acceptance",
    status_agendada_atrasada: "Scheduled / Overdue",
    status_agendada: "Scheduled",
    status_atrasada: "Overdue",
    sem_entrevista: "No interviews scheduled",
    msg_sem_entrevista: "Accepted interviews will appear here.",
    proxima_entrevista: "Upcoming Interviews",
    hoje: "Today",
    amanha: "Tomorrow",
    entrevista_agendada: "Scheduled Interviews",
    entrevista_pendente: "Pending / Overdue Interviews",
    domingo: "Sun",
    segunda: "Mon",
    terca: "Tue",
    quarta: "Wed",
    quinta: "Thu",
    sexta: "Fri",
    sabado: "Sat",
  },
  dash_candidato: {
    header: "My Dashboard",
    sub_header:
      "Track your selection processes, interviews, and profile progress.",
    skill_header: "My Profile",
    skill_titulo: "Compare your reported level with the evaluation received.",
    skill_hard: "Hard Skills",
    skill_soft: "Soft Skills",
    skill_perfil: "My Score",
    skill_avaliador: "Evaluator",
    skill_hard_descricao: "Technical skills",
    skill_soft_descricao: "Behavioral skills",
    sem_avaliacao: "Evaluation not available yet",
    erro: "Unable to load the dashboard.",
    stats_seletivo: "Selection Processes",
    stats_seletivo_msg: "In Progress",
    stats_agendada: "Scheduled appointments",
    stats_agendada_msg: "Confirmed",
    stats_realizada: "Completed Interviews",
    stats_realizada_msg: "Completed",
    stats_skill: "Evaluated Skills",
    stats_skill_msg: "With Evaluation",
    avaliacao_confirmada: "Evaluations confirmed with your evaluators.",
    sem_agenda: "You have no scheduled interviews.",
    agenda_skill: "Skill Evaluation",
    agenda_atrasada: "Overdue",
    agenda_confirmada: "Confirmed",
    agenda_processo: "Recruitment process",
    agenda_vaga: "Recruitment process",
    agenda_palestra_evento: "Talk / Event",
    agenda_mentoria: "Mentorship",
    agenda_projeto_consultoria: "Project / Consulting",
    agenda_networking: "Networking",
    agenda_outro: "Other",
    agenda_descricao: "Your upcoming confirmed appointments.",
    stats_oportunidades: "Other opportunities",
    stats_oportunidades_msg: "In progress",
    oportunidades_titulo: "My opportunities",
    oportunidades_descricao: "Track your opportunities in progress.",
    sem_oportunidades: "You have no opportunities in progress.",
    ver_todas_oportunidades: "View all",
    ver_processo: "View process",
    ver_oportunidade: "View opportunity",
    compatibilidade: "Compatibility",
    ultima_atualizacao: "Last updated",

    tipo_vaga: "Recruitment process",
    tipo_palestra_evento: "Talk / Event",
    tipo_mentoria: "Mentorship",
    tipo_projeto_consultoria: "Project / Consulting",
    tipo_networking: "Networking",
    tipo_outro: "Other",

    status_convite_aceito: "Invitation accepted",
    status_agenda_enviada: "Awaiting confirmation",
    status_agendado: "Interview scheduled",
    status_entrevista_realizada: "Interview completed",
    oportunidade_agendada: "Interview confirmed",
    movimentacoes_titulo: "Recent activity",
    movimentacoes_descricao:
      "Track the latest activity from your opportunities.",
    sem_movimentacoes: "No recent activity",
    sem_movimentacoes_msg: "Your next updates will appear here.",
    movimentacao_convite_recebido: "Invitation received",
    movimentacao_convite_aceito: "Invitation accepted",
    movimentacao_agenda_enviada: "New schedule received",
    movimentacao_agenda_confirmada: "Schedule confirmed",
    movimentacao_processo_finalizado: "Process completed",
  },
  dash_avaliador: {
    stats_pendente: "Pending Evaluations",
    stats_pendente_msg: "Awaiting Completion",
    stats_agendada: "Scheduled Interviews",
    stats_agendada_msg: "Confirmed",
    stats_skill: "Evaluated Skills",
    stats_skill_msg: "Total Completed",
    stats_pontuacao: "Score",
    stats_pontuacao_msg: "Current Score",

    header: "My Dashboard",
    sub_header:
      "Track your evaluations, interviews, and performance as an evaluator.",
    erro: "Unable to load the dashboard.",

    agenda_titulo: "Scheduled Interviews",
    agenda_descricao: "Confirmed evaluations with candidates.",
    agenda_atrasada: "Overdue",
    agenda_confirmada: "Confirmed",
    sem_agenda: "You have no scheduled interviews.",

    pendencias_titulo: "Evaluations Requiring Action",
    pendencias_descricao: "Evaluations that require action from you.",
    acao_questionario: "Send Questionnaire",
    acao_agenda: "Send Schedule",
    acao_reagendar: "Reschedule Interview",
    acao_finalizar: "Complete Evaluation",
    btn_ver_avaliacao: "View Evaluation",
    sem_pendencias: "No evaluations require your attention at this time.",

    top_skills_titulo: "Most Evaluated Skills",
    top_skills_descricao: "Skills you have evaluated most frequently.",
    sem_top_skills: "You don't have any completed evaluations yet.",

    ranking_titulo: "Score and Ranking",
    ranking_descricao:
      "Your score influences your priority for receiving evaluations.",
    ranking_pontuacao: "Current Score",
    ranking_pontos: "points",
    ranking_posicao: "Ranking Position",
    ranking_avaliadores: "evaluators",
  },
  perfil_candidato: {
    sobre: "About",
    modalidades: "Work arrangements",
    hard_skills: "Hard Skills",
    soft_skills: "Soft Skills",
    formacao: "Education",
    certificacoes: "Certifications",
    avaliado: "Evaluated",
    autoavaliacao: "Self-assessment",
    sem_apresentacao: "The candidate has not provided an introduction.",
    sem_modalidades: "No work arrangements provided.",
    sem_skills: "No skills provided.",
    sem_formacao: "No education information provided.",
    sem_certificacoes: "No certifications provided.",
    fechar: "Close",
    selecionar: "Select candidate",
    remover_selecao: "Remove selection",
    ver_perfil: "View profile",
    erro_carregar: "Unable to load the candidate profile.",
  },
  buscar_candidatos: {
    convite_titulo: "Invite candidates",
    convite_descricao:
      "Send a professional invitation to the selected candidates.",

    convite_candidato_selecionado: "candidate selected",
    convite_candidatos_selecionados: "candidates selected",
    convite_mesma_mensagem: "They will all receive the same invitation.",

    convite_motivo: "Invitation purpose",
    convite_motivo_descricao: "Select the context of the invitation.",

    convite_tipo_oportunidade: "Professional opportunity",
    convite_tipo_oportunidade_descricao:
      "Invitation regarding a professional opportunity.",

    convite_tipo_palestra: "Talk / event",
    convite_tipo_palestra_descricao:
      "Participation in a talk, event, or professional gathering.",

    convite_tipo_mentoria: "Mentoring",
    convite_tipo_mentoria_descricao:
      "Invitation related to mentoring or professional guidance.",

    convite_tipo_projeto: "Project / consulting",
    convite_tipo_projeto_descricao:
      "Participation in a project or consulting engagement.",

    convite_tipo_networking: "Networking / conversation",
    convite_tipo_networking_descricao:
      "Professional networking or an initial conversation.",

    convite_tipo_outro: "Other",
    convite_tipo_outro_descricao: "Another type of professional invitation.",

    convite_campo_titulo: "Title",
    convite_titulo_placeholder:
      "E.g.: Invitation to speak about project management",

    convite_campo_mensagem: "Message",
    convite_mensagem_placeholder:
      "Share some details about the invitation, its context, and why you would like to get in touch.",
    convite_mensagem_alerta:
      "Avoid including sensitive or confidential information.",

    convite_btn_cancelar: "Cancel",
    convite_btn_enviar: "Send invitation",

    convite_mock_sucesso_um: "Invitation prepared for 1 candidate.",
    convite_mock_sucesso_varios:
      "Invitation prepared for {{total}} candidates.",
    convite_btn_fechar: "Close",
    convite_conflito_titulo:
      "{{quantidade}} candidate(s) already have an active invitation of this type",
    convite_conflito_descricao:
      "These candidates will not receive another invitation while their current process is still in progress.",
    convite_ativo: "Active invitation",
    convite_btn_enviar_quantidade: "Send to {{quantidade}} candidate(s)",
    convite_enviando: "Sending...",
  },
  oportunidades_candidato: {
    titulo: "My opportunities",
    descricao: "Track your invitations, interviews, and completed processes.",
    convites: "Invitations",
    entrevistas: "Interviews",
    finalizados: "Completed",

    recebido_em: "Received on",
    finalizado_em: "Completed on",

    btn_aceitar: "Accept invitation",
    btn_recusar: "Decline",
    btn_aceitar_horario: "Accept time",
    btn_recusar_horario: "Decline time",

    aguardando_resposta_agenda: "Awaiting your confirmation",

    aprovado: "Approved",
    nao_aprovado: "Not approved",
    parecer: "Recruiter feedback",

    tipo_vaga: "Job",
    tipo_oportunidade: "Opportunity",
    tipo_evento: "Talk / Event",
    tipo_mentoria: "Mentorship",
    tipo_projeto: "Project / Consulting",
    tipo_networking: "Networking",
    tipo_outro: "Other",
    carregando_convites: "Loading your invitations...",
    erro_convites: "We couldn't load your invitations.",
    sem_convites: "No pending invitations",
    sem_convites_msg: "When you receive a new invitation, it will appear here.",
    horario_recusado: "Time declined",
    aguardando_nova_agenda:
      "Waiting for a new time suggestion from the recruiter.",
    aguardando_agenda: "Waiting for the recruiter to suggest a time.",
    entrevista_confirmada: "Interview confirmed",
    entrevista_realizada: "Interview completed",
    aguardando_finalizacao:
      "Interview completed. Waiting for the recruiter to finalize the process.",
    carregando_finalizados: "Loading completed processes...",
    erro_finalizados: "We couldn't load the completed processes.",
    sem_finalizados: "No completed processes",
    sem_finalizados_msg: "Your completed processes will appear here.",
    processo_finalizado: "Process completed",
    carregando_entrevistas: "Loading your interviews...",
    erro_entrevistas: "We couldn't load your interviews.",
    sem_entrevistas: "No interviews in progress",
    sem_entrevistas_msg:
      "When you accept an invitation, the process progress will appear here.",
  },
  processos_recrutador: {
    titulo: "Processes",
    descricao: "Manage invitations, interviews, and candidate processes.",

    em_andamento: "In progress",
    finalizados: "Completed",
    recusados: "Declined",

    tipo_vaga: "Job",
    tipo_oportunidade: "Opportunity",
    tipo_evento: "Talk / Event",
    tipo_mentoria: "Mentorship",
    tipo_projeto: "Project / Consulting",
    tipo_networking: "Networking",
    tipo_outro: "Other",

    convite_aceito: "Invitation accepted",
    convite_recusado: "Invitation declined",

    sugerir_entrevista_msg:
      "The candidate accepted the invitation. Suggest an interview date and time.",
    horario_recusado: "Time declined",
    sugerir_novo_horario_msg:
      "The candidate declined the suggested time. Send a new option.",

    aguardando_confirmacao_candidato: "Awaiting candidate confirmation",
    entrevista_confirmada: "Interview confirmed",
    entrevista_realizada: "Interview completed",
    finalizar_processo_msg:
      "The interview has been completed. The process can now be finalized.",

    btn_sugerir_entrevista: "Suggest interview",
    btn_novo_horario: "Suggest new time",
    btn_realizada: "Mark as completed",
    btn_finalizar: "Finalize process",

    aprovado: "Approved",
    nao_aprovado: "Not approved",
    parecer: "Feedback for the candidate",

    finalizado_em: "Completed on",
    recusado_em: "Declined on",
    modal_agenda: {
      titulo: "Suggest interview",
      titulo_novo: "Suggest a new time",
      descricao: "Choose a date and time for the interview with the candidate.",
      data: "Interview date",
      hora: "Time",
      aviso:
        "The candidate will receive the suggestion and can accept or decline the proposed time.",
      horario_anterior_recusado: "Previous time declined by the candidate",
      btn_cancelar: "Cancel",
      btn_enviar: "Send suggestion",
      btn_novo: "Send new time",
      data_invalida: "Select a future date and time.",
      btn_enviando: "Sending...",
      erro: "Unable to send the interview suggestion.",
    },
    sucesso_marcar_realizada: "Interview marked as completed successfully.",
    erro_marcar_realizada: "Unable to mark the interview as completed.",
    btn_marcando_realizada: "Updating...",
    modal_finalizacao: {
      titulo: "Finalize process",
      descricao:
        "Enter the interview outcome and share feedback with the candidate.",
      resultado: "Process outcome",
      aprovado: "Approved",
      nao_aprovado: "Not approved",
      feedback: "Feedback for the candidate",
      feedback_placeholder:
        "Write feedback about the candidate's participation in the process...",
      feedback_aviso: "This feedback will be visible to the candidate.",
      btn_cancelar: "Cancel",
      btn_finalizar: "Finalize process",
      btn_finalizando: "Finalizing...",
      sucesso: "Process finalized successfully.",
      erro: "Unable to finalize the process.",
    },
    convites: "Invitations",
    carregando_convites: "Loading invitations...",
    sem_convites: "No invitations awaiting response",
    sem_convites_descricao:
      "Sent invitations that are still awaiting a response from the candidate will appear here.",
    aguardando_resposta: "Awaiting response",
    enviado_em: "Sent on",
    erro_carregar_convites: "Unable to load invitations.",
  },
  dash_recrutador: {
    header: "My Dashboard",
    sub_header:
      "Track your invitations, interviews, and recruitment processes.",
    stats_convites: "Invitations sent",
    stats_convites_msg: "Awaiting response",
    stats_andamento: "Processes in progress",
    stats_andamento_msg: "Active processes",
    stats_agendada: "Scheduled interviews",
    stats_agendada_msg: "Confirmed",
    stats_finalizados: "Completed processes",
    stats_finalizados_msg: "Total completed",
    agenda_titulo: "Scheduled interviews",
    agenda_descricao: "Upcoming confirmed interviews with candidates.",
    agenda_confirmada: "Confirmed",
    agenda_atrasada: "Overdue",
    sem_agenda: "No interviews scheduled.",
    pendencias_titulo: "Processes requiring action",
    pendencias_descricao: "Processes waiting for your action to continue.",
    acao_agenda: "Suggest interview",
    acao_finalizar: "Finalize process",
    btn_ver_processo: "View process",
    sem_pendencias: "No processes are waiting for your action.",
    stats_vagas: "Open positions",
    stats_vagas_msg: "Available to candidates",
    erro: "Unable to load the dashboard.",
  },
};

export default en;
