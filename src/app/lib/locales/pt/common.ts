const pt = {
  notfound: {
    primeira_linha: "Página não encontrada",
    segunda_linha:
      "A página que você tentou acessar não existe ou o perfil informado é inválido.",
    btn_voltar: "Voltar à página inicial",
  },
  header: {
    mnu_primeiro: "Como Funciona",
    mnu_segundo: "Sobre Nós",
    mnu_terceiro: "Comece Agora",
    mnu_planos: "Planos",
  },
  pages_home: {
    titulo_funciona: "Como Funciona",
    titulo_sobre: "Sobre Nós",
    titulo_planos: "Nossos Planos",
  },
  home: {
    titulo: "As melhores oportunidades",
    subtitulo: "para as melhores pessoas e empresas",
    pefil_empresa: "Empresa",
    pefil_empresa_texto:
      "Os melhores especialistas e os profissionais certos para sua empresa.",
    pefil_candidato: "Candidato",
    pefil_candidato_texto:
      "Acesso às oportunidades que combinam com seu perfil.",
    pefil_especialista: "Especialista",
    pefil_especialista_texto:
      "Visibilidade como profissional para empresas em crescimento.",
  },
  login: {
    mnu_entrar: "Entrar",
    mnu_criar: "Criar Conta",
    email: "E-mail",
    placehold_email: "Digite seu e-mail",
    senha: "Senha",
    placehold_senha: "Digite sua senha",
    btn_entrar: "Entrar",
    link_esqueceu_senha: "Esqueceu sua senha?",
    carregando: "Carregando...",
    erro_senha: "Credenciais inválidas",
    erro_campos: "Email e senha são obrigatórios.",
  },
  esqueceu: {
    btn_reenviar: "Reenviar Senha",
    sucesso: "Se o e-mail existir, um link será enviado ao email informado.",
  },
  cadastro: {
    nome: "Nome",
    placehold_nome: "Primeiro nome",
    sobrenome: "Sobrenome",
    placehold_sobrenome: "Ultimo nome",
    placehold_senha: "Crie uma senha",
    repete_senha: "Repete senha",
    placehold_repete_senha: "Repita sua senha",
    erro_repete_senha: "As senhas não coincidem.",
    btn: "Cadastrar Conta",
    sucesso:
      "Cadastro efetuado com sucesso, acesse seu e-mail para ativar o cadastro.",
    senha_invalida:
      "A senha deve ter entre 8 e 16 caracteres, com letra maiúscula, minúscula, número e caractere especial.",
    data_nascimento: "Data Nascimento",
    placehold_data_nascimento: "Dt. de Nascimento",
    sexo: "Gênero",
    placehold_sexo: "Selecione o gênero",
    nome_social: "Nome Social",
    placehold_nome_social: "Informe seu nome social se desejar",
    tooltip_msg_nome_social:
      "Caso você informe um Nome Social, ele será respeitado e utilizado como seu nome principal na plataforma.",
    item_alerta_erro_buscar_dados: "Erro ao buscar os dados",
    estado: "Estado",
    placehold_estado: "Selecione o Estado",
    cidade: "Cidade",
    placehold_cidade: "Selecione a Cidade",
    data_invalida: "Informe uma data válida.",
  },
  redefinir: {
    senha_erro: "A senha deve ter pelo menos 6 caracteres.",
    token_invalido: "Token inválido ou ausente.",
    erro_geral: "Erro ao redefinir a senha. Tente novamente.",
    nova_senha: "Nova Senha",
    nova_senha_placehold: "Digite sua nova senha",
    btn_redefinir: "Redefinir Senha",
    redefinir_carregando: "Redefinindo...",
  },
  confirma_email: {
    token_invalido: "Token inválido ou ausente.",
    erro_geral: "Erro ao ativar a conta. Token inválido ou expirado.",
    reenvio_link_sucesso: "Link reenviado com sucesso. Verifique seu e-mail.",
    reenvio_link_erro: "Erro ao reenviar o link. Tente novamente.",
    titulo: "Confirmação de E-mail",
    confirmando: "Confirmando seu e-mail...",
    redirecionando: "Redirecionando para o login...",
    reenviar: "Reenviando...",
    btn_reenviar: "Reenviar link de ativação",
    info: "Para reenviar o link, acesse a página de login.",
  },
  confirma_email_avaliador: {
    token_invalido: "Token inválido ou ausente.",
    erro_geral: "Erro ao ativar a conta. Token inválido ou expirado.",
    reenvio_link_sucesso: "Link reenviado com sucesso. Verifique seu e-mail.",
    reenvio_link_erro: "Erro ao reenviar o link. Tente novamente.",
    titulo: "Confirmação de Cadastro",
    titulo_reject: "Cancelamento de Cadastro",
    confirmando: "Confirmando seu cadastro...",
    redirecionando: "Redirecionando para o login...",
    reenviar: "Reenviando...",
    btn_reenviar: "Reenviar link de ativação",
    info: "Para reenviar o link, acesse a página de login.",
  },
  perfil: {
    candidato: "Candidato(a)",
    recrutador: "Recrutador(a)",
    avaliador: "Avaliador(a)",
    titulo_primeiro: "Olá, seja bem vindo(a) ao Whizzat!",
    titulo_segundo: "Para iniciar, escolha uma opção de perfil",
    titulo_terceiro:
      "Mas fique tranquilo(a), você pode criar outro perfil após acessar o Whizzat",
    card_candidato: "Sou um(a) Candidato(a)",
    card_candidato_descricao:
      "Estou à procura da melhor oportunidade profissional",
    card_recrutador: "Sou um(a) Recrutador(a)",
    card_recrutador_descricao:
      "Estou à procura do melhor candidato para uma vaga",
    card_avaliador: "Sou um(a) Avaliador(a)",
    card_avaliador_descricao:
      "Desejo avaliar candidatos para receber benefícios",
    btn_sair: "Sair",
    selecionado_ola: "Olá, ",
    selecionado_candidato_titulo: "Olá, Candidato(a)",
    selecionado_candidato_descricao_curta: "Sou um(a) Candidato(a)",
    selecionado_candidato_cor: "verde",
    selecionado_candidato_cor_css: "green",
    selecionado_recrutador_titulo: "Olá, Recrutador(a)",
    selecionado_recrutador_descricao_curta: "Sou um(a) Recrutador(a)",

    selecionado_recrutador_cor: "roxo",
    selecionado_recrutador_cor_css: "purple",
    selecionado_avaliador_titulo: "Olá, Avaliador(a)",
    selecionado_avaliador_descricao_curta: "Sou um(a) Avaliador(a)",
    selecionado_avaliador_cor: "azul",
    selecionado_avaliador_cor_css: "blue",
    selecionado_titulo_primeiro:
      "Nós só vamos pedir os dados que forem necessários.",
    selecionado_titulo_segundo:
      "Nossa política é somente perguntar o que for estritamente necessário então fique tranquilo(a).",
    selecionado_titulo_terceiro:
      "À medida que você for efetuando ações no Whizzat, vamos pedindo informações, claro se isso for indispensável.",
    selecionado_titulo_quarto:
      "No Whizzat os perfis de usuário são identificados por cores.",
    selecionado_titulo_quinto: "No perfil de",
    selecionado_titulo_sexto: "a cor predominante é",
    btn_voltar: "Voltar",
    btn_pular_tutorial: "Pular tutorial",
    btn_avancar: "Avançar",
    selecionado_invalido: "Perfil inválido",
  },
  semdados: {
    empresa_linha_primeiro:
      "Olá! Sua empresa ainda não possui uma página cadastrada.",
    empresa_linha_segundo:
      "Complete os dados da sua empresa para poder criar anúncios de vagas e receber sugestões de candidatos compatíveis.",
    empresa_linha_terceiro:
      "Você poderá cadastrar quantas empresas quiser. Clique no botão abaixo para começar.",
    empresa_linha_botao: "Preencher dados da empresa",
    vaga_linha_primeiro: "Nenhuma vaga cadastrada até o momento.",
    vaga_linha_segundo:
      "Crie a primeira oportunidade clicando no botão abaixo.",
    vaga_linha_botao: "Cadastrar vaga",
    perfil_linha_primeiro: "Olá! Seu perfil de ",
    perfil_linha_primeiro_complemento: "ainda não está completo ou ativo.",
    perfil_linha_segundo:
      "Ative ou complete os dados para poder utilizar os recursos do seu perfil.",
    perfil_linha_botao: "Verificar meu perfil",
    candidato_ignorado_linha_primeiro: "Nenhum candidato ignorado",
    candidato_ignorado_linha_segundo:
      "Os candidatos que você escolher não mostrar nas buscas aparecerão aqui.",
  },
  sidebar: {
    menu_dashboard: "Dashboard",
    menu_avaliacoes: "Minhas Avaliações",
    menu_convite: "Convite/Avaliar",
    menu_meu_perfil: "Meu Perfil",
    menu_agenda: "Agenda",
    menu_notificacoes: "Notificações",
    menu_vagas_abertas: "Vagas Abertas",
    menu_empresas: "Empresas",
    menu_avaliadores: "Avaliadores",
    menu_minhas_vagas: "Minhas Vagas",
    menu_buscar_vagas: "Buscar Vagas",
    menu_sair: "Sair",
    menu_questionario: "Questionarios",
    menu_candidatos_ignorados: "Candidatos ignorados",
    menu_buscar_candidatos: "Buscar candidatos",
    minhas_oportunidades: "Minhas oportunidades",
    menu_processos: "Processos",
  },
  tela_topo_passos: {
    passo_dados: "Dados",
    passo_formacao: "Formação",
    passo_certificacao: "Certificação",
    passo_hardskills: "Hard-Skills",
    passo_softskills: "Soft-Skills",
    passo_visualizar: "Visualizar",
    passo_publicar: "Publicar",
    passo_localizacao: "Localização",
    passo_apresentacao: "Apresentação",
  },
  tela_perfil_avaliador: {
    item_ativo: "Ativo",
    item_inativo: "Inativo",
    item_label_situacao_cadastro: "Situação do Cadastro",
    item_label_enviando: "Enviando ...",
    item_msg_situacao_cadastro_aguardando: "Aguardando confirmação",
    item_msg_situacao_cadastro_confirmado: "Aprovado",
    item_msg_situacao_cadastro_rejeitado: "Cadastro Reprovado",
    item_botao_situacao_cadastro: "Reenviar Solicitação",
    item_label_nome: "Nome",
    item_placeholder_nome: "Nome",
    item_label_empresa_whizzat:
      "Você trabalha para uma empresa que está na Whizzat?",
    item_msg_empresa_whizzat_sim: "Sim",
    item_msg_empresa_whizzat_nao: "Não",
    item_label_empresa: "Empresa:",
    item_msg_empresa: "Selecione a empresa:",
    item_msg_sem_empresa: "Sem empresa",
    item_label_avaliar_vagas: "Você deseja avaliar candidatos e vagas:",
    item_msg_avaliar_vagas_todas: "De qualquer empresa",
    item_msg_avaliar_todas: "Avaliar todos",
    item_msg_avaliar_vagas_minha: "Somente da minha empresa",
    item_msg_avaliar_minha: "Avaliar somente da minha empresa",
    item_label_telefone: "Telefone de contato:",
    item_placeholder_telefone: "Telefone",
    item_label_localizacao: "Localização:",
    item_placeholder_localizacao: "Informe o seu local",
    item_label_notificacao: "Meio de Notificação:",
    item_msg_notificacao_whatsapp: "WhatsApp",
    item_msg_notificacao_sms: "SMS",
    item_label_descreva:
      "Descreva, em algumas linhas, um pouco mais sobre você. Este texto será sua carta de apresentação para as empresas na plataforma.",
    item_label_descreva_negrito:
      "(Caso você seja de uma única empresa, pode deixar em branco este campo)",
    item_placeholder_apresentacao: "Apresente-se",
    item_msg_nenhuma_apresentacao: "Nenhuma apresentação fornecida.",
    item_label_foto:
      "Carregue uma foto, imagem, logo do avaliador (recomendado 512×512 px, até 1MB):",
    item_msg_foto: "Clique aqui e carregue sua imagem",
    item_msg_sem_foto: "Sem logo",
    item_label_informe_formacao: "Informe as suas formações acadêmicas",
    item_label_graduacao: "Graduação:",
    item_msg_graduacao: "Selecione a graduação",
    item_label_formacao: "Formação:",
    item_msg_formacao: "Digite sua formação",
    item_label_certificado: "Certificado (JPG, PNG ou PDF):",
    item_msg_certificado: "Selecione o arquivo",
    item_botao_adicionar: "+ Adicionar",
    item_botao_visualizar: "Visualizar Certificado",
    item_botao_substituir: "Substituir Certificado",
    item_msg_nenhuma_graduacao: "Graduação não encontrada",
    item_botao_remover_formacao: "Remover Formação",
    item_label_informe_certificacao: "Informe as suas certificações",
    item_label_certificacao: "Certificações:",
    item_msg_certificacao: "Digite ou selecione",
    item_msg_criar_certificacao: "Criar nova certificação:",
    item_botao_remover_certificacao: "Remover Certificação",
    item_botao_certificado_atualizado: "Certificado Atualizado!",
    item_label_informe_hardskills:
      "Informe as Hard Skills que você deseja atuar como avaliador",
    item_label_informe_hardskills_subitem:
      "Hard Skill são as competência técnicas adquiridas pelo profissional ao longo da sua carreira.",
    item_label_informe_softskills:
      "Informe as Soft Skills que você deseja atuar como avaliador",
    item_label_informe_softskills_subitem:
      "Soft Skills são as competência comportamentais naturais ou adquiridas pelo profissional ao longo da sua carreira.",
    item_label_informe_qtde: "(mínimo 3 e no máximo 12)",
    item_label_skill: "Skills:",
    item_msg_skill: "Digite ou selecione uma skill",
    item_msg_criar_skill: "Criar nova skill:",
    item_label_peso: "Nível",
    item_label_favorito: "Quero ser um avaliador:",
    item_placeholder_favorito: "Tempo skill",
    item_botao_remover_skill: "Remover Skill",
    item_label_lista_formacao: "Formações",
    item_msg_nenhuma_formacao: "Nenhuma formação cadastrada",
    item_label_lista_certificacao: "Certificações",
    item_msg_nenhuma_certificacao: "Nenhuma certificação cadastrada",
    item_botao_voltar: "Voltar",
    item_botao_cancelar: "Cancelar",
    item_botao_avancar: "Avançar",
    item_botao_publicar: "Publicar",
    item_msg_campo_obt: "Campo Obrigatório",
    item_msg_logo_obt: "Logo Obrigatório",
    item_alerta_descartada: "Alterações descartadas.",
    item_alerta_sucesso: "Avaliador publicado com sucesso!",
    item_alerta_erro_buscar_dados: "Erro ao buscar dados do avaliador",
    item_alerta_erro_tipo_arq:
      "Envie apenas arquivos de imagem (JPG, PNG, SVG, etc.)",
    item_alerta_erro_tamanho_arq: "O arquivo deve ter no máximo 1MB.",
    item_alerta_erro_tamanho_arq2: "O arquivo deve ter no máximo 2MB.",
    item_alerta_erro_salvar: "Erro ao salvar avaliador.",
    item_alerta_erro_reenviar_solicitacao: "Erro ao reenviar solicitação",
    item_alerta_reenviar_sucesso: "Solicitação reenviada com sucesso!",
    item_tooltip_certificacao_titulo:
      "Como adicionar certificação que não está na lista:",
    item_tooltip_certificacao_passo1: "1. Digite a certificação desejada;",
    item_tooltip_certificacao_passo2: "2. Selecione 'Criar nova certificação';",
    item_tooltip_certificacao_passo3: "3. Clique no botão Adicionar.",
    item_tooltip_skill_titulo: "Como adicionar skill que não está na lista:",
    item_tooltip_skill_passo1: "1. Digite a skill desejada;",
    item_tooltip_skill_passo2: "2. Selecione 'Criar nova skill';",
    item_tooltip_skill_passo3: "3. Clique no botão Adicionar.",
    item_tooltip_favorite_passo1:
      "- Pode ser selecionado até 6 skills para ser um avaliador.",
    item_tooltip_favorite_passo2: "- Os Skills marcados serão assumidos ",
    item_tooltip_favorite_passo2_1: " para avaliações de candidatos.",
  },
  tela_perfil_recrutador: {
    item_ativo: "Ativo",
    item_inativo: "Inativo",
    item_label_nome: "Nome",
    item_placeholder_primeiro_nome: "Primeiro Nome",
    item_placeholder_ultimo_nome: "Ultimo Nome",
    item_label_telefone: "Telefone de contato:",
    item_placeholder_telefone: "Telefone",
    item_label_localizacao: "Localização:",
    item_placeholder_localizacao: "Informe o seu local",
    item_label_notificacao: "Meio de Notificação:",
    item_msg_notificacao_whatsapp: "WhatsApp",
    item_msg_notificacao_sms: "SMS",
    item_label_descreva:
      "Descreva, em algumas linhas, um pouco mais sobre você. Este texto será sua carta de apresentação para as empresas na plataforma.",
    item_label_descreva_negrito:
      "(Caso você seja de uma única empresa, pode deixar em branco este campo)",
    item_placeholder_apresentacao: "Apresente-se",
    item_msg_nenhuma_apresentacao: "Nenhuma apresentação fornecida.",
    item_label_foto:
      "Carregue uma foto, imagem, logo do recrutador (recomendado 512×512 px, até 1MB):",
    item_msg_foto: "Clique aqui e carregue sua imagem",
    item_msg_sem_foto: "Sem logo",
    item_botao_voltar: "Voltar",
    item_botao_cancelar: "Cancelar",
    item_botao_avancar: "Avançar",
    item_botao_publicar: "Publicar",
    item_msg_campo_obt: "Campo Obrigatório",
    item_msg_logo_obt: "Logo Obrigatório",
    item_alerta_descartada: "Alterações descartadas.",
    item_alerta_sucesso: "Recrutador publicado com sucesso!",
    item_alerta_erro_buscar_dados: "Erro ao buscar dados do recrutador",
    item_alerta_erro_tipo_arq:
      "Envie apenas arquivos de imagem (JPG, PNG, SVG, etc.)",
    item_alerta_erro_tamanho_arq: "O arquivo deve ter no máximo 1MB.",
    item_alerta_erro_salvar: "Erro ao salvar recrutador.",
  },
  tela_perfil_candidato: {
    item_ativo: "Ativo",
    item_inativo: "Inativo",
    item_label_nome: "Nome",
    item_placeholder_nome: "Nome",
    item_label_telefone: "Telefone de contato:",
    item_placeholder_telefone: "Telefone",
    item_label_localizacao: "Localização:",
    item_placeholder_localizacao: "Informe o seu local",
    item_label_notificacao: "Meio de Notificação:",
    item_msg_notificacao_whatsapp: "WhatsApp",
    item_msg_notificacao_sms: "SMS",
    item_label_descreva:
      "Descreva, em algumas linhas, um pouco mais sobre você. Este texto será sua carta de apresentação para as empresas na plataforma.",
    item_placeholder_apresentacao: "Apresente-se",
    item_msg_nenhuma_apresentacao: "Nenhuma apresentação fornecida.",
    item_label_foto:
      "Carregue uma foto, imagem, logo do candidato (recomendado 512×512 px, até 1MB):",
    item_msg_foto: "Clique aqui e carregue sua imagem",
    item_msg_sem_foto: "Sem logo",
    item_label_informe_formacao: "Informe as suas formações acadêmicas",
    item_label_graduacao: "Graduação:",
    item_msg_graduacao: "Selecione a graduação",
    item_label_formacao: "Formação:",
    item_msg_formacao: "Digite sua formação",
    item_label_certificado: "Certificado (JPG, PNG ou PDF):",
    item_msg_certificado: "Selecione o arquivo",
    item_botao_adicionar: "+ Adicionar",
    item_botao_visualizar: "Visualizar Certificado",
    item_botao_substituir: "Substituir Certificado",
    item_msg_nenhuma_graduacao: "Graduação não encontrada",
    item_botao_remover_formacao: "Remover Formação",
    item_label_informe_certificacao: "Informe as suas certificações",
    item_label_certificacao: "Certificações:",
    item_msg_certificacao: "Digite ou selecione",
    item_msg_criar_certificacao: "Criar nova certificação:",
    item_botao_remover_certificacao: "Remover Certificação",
    item_botao_certificado_atualizado: "Certificado Atualizado!",
    item_label_informe_hardskills:
      "Informe as Hard Skills que você deseja atuar como candidato",
    item_label_informe_hardskills_subitem:
      "Hard Skill são as competência técnicas adquiridas pelo profissional ao longo da sua carreira.",
    item_label_informe_softskills:
      "Informe as Soft Skills que você deseja atuar como candidato",
    item_label_informe_softskills_subitem:
      "Soft Skills são as competência comportamentais naturais ou adquiridas pelo profissional ao longo da sua carreira.",
    item_label_informe_qtde: "(mínimo 3 e no máximo 12)",
    item_label_skill: "Skills:",
    item_msg_skill: "Digite ou selecione uma skill",
    item_msg_criar_skill: "Criar nova skill:",
    item_label_peso: "Nível",
    item_label_favorito: "Favorito",
    item_placeholder_favorito: "Tempo skill",
    item_botao_remover_skill: "Remover Skill",
    item_label_lista_formacao: "Formações",
    item_msg_nenhuma_formacao: "Nenhuma formação cadastrada",
    item_label_lista_certificacao: "Certificações",
    item_msg_nenhuma_certificacao: "Nenhuma certificação cadastrada",
    item_botao_voltar: "Voltar",
    item_botao_cancelar: "Cancelar",
    item_botao_avancar: "Avançar",
    item_botao_publicar: "Publicar",
    item_msg_campo_obt: "Campo Obrigatório",
    item_msg_logo_obt: "Logo Obrigatório",
    item_alerta_descartada: "Alterações descartadas.",
    item_alerta_sucesso: "candidato publicado com sucesso!",
    item_alerta_erro_buscar_dados: "Erro ao buscar dados do candidato",
    item_alerta_erro_tipo_arq:
      "Envie apenas arquivos de imagem (JPG, PNG, SVG, etc.)",
    item_alerta_erro_tamanho_arq: "O arquivo deve ter no máximo 1MB.",
    item_alerta_erro_tamanho_arq2: "O arquivo deve ter no máximo 2MB.",
    item_alerta_erro_salvar: "Erro ao salvar candidato.",
    item_tooltip_certificacao_titulo:
      "Como adicionar certificação que não está na lista:",
    item_tooltip_certificacao_passo1: "1. Digite a certificação desejada;",
    item_tooltip_certificacao_passo2: "2. Selecione 'Criar nova certificação';",
    item_tooltip_certificacao_passo3: "3. Clique no botão Adicionar.",
    item_tooltip_skill_titulo: "Como adicionar skill que não está na lista:",
    item_tooltip_skill_passo1: "1. Digite a skill desejada;",
    item_tooltip_skill_passo2: "2. Selecione 'Criar nova skill';",
    item_tooltip_skill_passo3: "3. Clique no botão Adicionar.",
    item_tooltip_favorite_passo1: "- Pode ser selecionado até 5 favoritos.",
    item_tooltip_favorite_passo2: "- Os Skills favoritos serão assumidos ",
    item_tooltip_favorite_passo2_1:
      " como preferenciais para avaliações de candidatos.",
    item_sugerido_avaliador: "Sugerido pelo avaliador:",
    aberto_oportunidades: "Aberto a oportunidades",
    disponivel_oportunidades: "Disponível para novas oportunidades",
    indisponivel_oportunidades: "Não disponível no momento",
    inclusao_titulo: "Oportunidades inclusivas e afirmativas",
    inclusao_descricao:
      "Estas opções são voluntárias e ajudam a identificar oportunidades inclusivas ou ações afirmativas compatíveis com você.",
    inclusao_pcd: "Oportunidades para pessoas com deficiência (PCD)",
    inclusao_pcd_msg:
      "Desejo participar de oportunidades destinadas a pessoas com deficiência.",
    inclusao_racial: "Ações afirmativas étnico-raciais",
    inclusao_racial_msg:
      "Desejo participar de oportunidades com políticas afirmativas étnico-raciais.",
    inclusao_lgbtqia: "Diversidade LGBTQIA+",
    inclusao_lgbtqia_msg:
      "Desejo participar de oportunidades voltadas à diversidade LGBTQIA+.",
    inclusao_50mais: "Profissionais 50+",
    inclusao_50mais_msg:
      "Desejo participar de oportunidades voltadas a profissionais com 50 anos ou mais.",
    inclusao_diversidade: "Outras iniciativas de diversidade e inclusão",
    inclusao_diversidade_msg:
      "Desejo ser considerado em outras oportunidades inclusivas compatíveis com meu perfil.",
    inclusao_privacidade:
      "O preenchimento é opcional. Estas preferências serão utilizadas somente para identificar oportunidades inclusivas ou afirmativas e não interferem na participação em vagas de ampla concorrência.",
    inclusao_visualizacao_msg:
      "Preferências informadas para participação em oportunidades inclusivas ou afirmativas.",
    inclusao_pcd_curto: "PCD",
    inclusao_racial_curto: "Ações afirmativas étnico-raciais",
    inclusao_lgbtqia_curto: "Diversidade LGBTQIA+",
    inclusao_50mais_curto: "Profissionais 50+",
    inclusao_diversidade_curto: "Diversidade e inclusão",
    inclusao_nenhuma_selecionada:
      "Nenhuma preferência de oportunidade inclusiva foi selecionada.",
    inclusao_privacidade_resumo:
      "Estas informações são opcionais e serão utilizadas somente para identificar oportunidades compatíveis.",
    modalidade_titulo: "Modalidade de trabalho",
    modalidade_descricao:
      "Selecione as modalidades em que você aceita receber oportunidades.",
    modalidade_presencial: "Presencial",
    modalidade_hibrido: "Híbrido",
    modalidade_remoto: "Remoto",
    modalidade_visualizacao_msg:
      "Modalidades de trabalho que você aceita para novas oportunidades.",
    modalidade_nenhuma_selecionada:
      "Nenhuma modalidade de trabalho foi selecionada.",
  },
  tela_lista_avaliadores: {
    item_titulo_aguardando: "Avaliadores Aguardando Confirmação",
    item_titulo_reprovados: "Avaliadores Reprovados",
    item_titulo_aprovados: "Avaliadores Aprovados",
    item_msg_sem_especialista: "Nenhum avaliador nesta categoria.",
    item_msg_sem_foto: "Sem logo",
    item_botao_aprovar: "Aprovar",
    item_botao_reprovar: "Reprovar",
    item_alerta_sucesso: "Avaliador atualizado com sucesso!",
    item_alerta_erro_buscar_dados: "Erro ao buscar Avaliador",
    item_alerta_erro_salvar: "Problemas ao tentar executar a operação!",
  },
  tela_lista_empresas: {
    item_ativo: "Ativa",
    item_inativo: "Inativa",
    item_msg_sem_foto: "Sem logo",
    item_botao_cadastrar: "+ Cadastrar Empresa",
    item_alerta_erro_buscar_dados: "Erro ao buscar Empresas",
    item_msg_sem_email: "Sem email",
    item_msg_sem_telefone: "Sem telefone",
  },
  tela_empresa_dados: {
    item_ativo: "Ativa",
    item_inativo: "Inativa",
    item_label_nome: "Nome da empresa",
    item_placeholder_nome: "Empresa",
    item_label_site: "Website da empresa",
    item_placeholder_site: "Website",
    item_label_email: "Email de contato",
    item_placeholder_email: "Email",
    item_label_telefone: "Telefone de contato:",
    item_placeholder_telefone: "Telefone",
    item_label_titulo_localizacao:
      "Informe sua localização para encontrar os melhores candidatos",
    item_label_localizacao: "Localização:",
    item_placeholder_localizacao: "Informe o local da sua empresa",
    item_label_titulo_descreva: "Apresentação da empresa",
    item_label_descreva:
      "Descreva, em algumas linhas, quem é sua empresa e o que ela busca profissionalmente. Este texto será sua carta de apresentação para candidatos e especialistas na plataforma:",

    item_placeholder_apresentacao: "Apresente sua empresa",
    item_msg_nenhuma_apresentacao: "Nenhuma apresentação fornecida.",
    item_label_foto:
      "Carregue o logotipo da sua empresa (recomendado 512×512 px, até 1MB):",
    item_msg_foto: "Clique aqui e carregue sua imagem",
    item_label_capa:
      "Carregue a imagem de capa da sua empresa (recomendado: 1200×300 px, até 1MB):",
    item_msg_sem_foto: "Sem logo",
    item_msg_sem_capa: "Sem imagem de capa",
    item_botao_voltar: "Voltar",
    item_botao_cancelar: "Cancelar",
    item_botao_avancar: "Avançar",
    item_botao_publicar: "Publicar",
    item_msg_campo_obt: "Campo Obrigatório",
    item_msg_logo_obt: "Logo Obrigatório",
    item_msg_capa_obt: "Capa Obrigatória",
    item_alerta_descartada: "Alterações descartadas.",
    item_alerta_sucesso_1: "Empresa ",
    item_alerta_sucesso_2: "publicada com sucesso!",
    item_alerta_erro_buscar_dados: "Erro ao buscar dados da empresa",
    item_alerta_erro_tipo_arq:
      "Envie apenas arquivos de imagem (JPG, PNG, SVG, etc.)",
    item_alerta_erro_tamanho_arq: "O arquivo deve ter no máximo 1MB.",
    item_alerta_erro_salvar: "Erro ao salvar empresa.",
    item_botao_lista_empresa: "← Voltar para lista de empresas",
    item_botao_cadastrar: "+ Cadastrar Empresa",
    item_botao_editar: "Editar Empresa",
    item_botao_vaga: "+ Cadastrar Vaga",
    item_item_vagas: "Vagas",
    item_item_periodo: "Aberta até:",
  },
  tela_lista_vagas: {
    item_titulo_sugeridos: "Candidatos sugeridos: ",
    item_titulo_escolhidos: "Candidatos escolhidos:",
    item_titulo_avaliacao: "Candidatos em avaliação:",
    item_msg_empresas: "Todas as empresas",
    item_msg_skills: "Todas as Skills",
    item_msg_modalidades: "Todas as modalidades",
    item_item_periodo: "Aberta até:",
    item_msg_sem_foto: "Sem logo",
    item_botao_filtrar: "Filtrar",
    item_botao_cadastrar: "+ Cadastrar Vagas",
    item_alerta_erro_buscar_dados: "Erro ao buscar vagas",
    item_msg_sem_vagas:
      "Nenhuma vaga disponível com os critérios selecionados.",
    item_msg_sem_vagas_filtro:
      "Tente ajustar os filtros ou limpar a seleção para ver mais resultados.",
    item_msg_item_mulher: "Mulheres",
  },
  tela_vaga_dados: {
    item_ativo: "Ativa",
    item_inativo: "Inativa",
    item_situacao: "Situação:",
    item_label_nome: "Empresa:",
    item_placeholder_nome: "Selecione a empresa",
    item_msg_indefinida: "Indefinida",
    item_label_site: "Titulo da vaga:",
    item_placeholder_site: "Titulo da vaga",
    item_label_local: "Local da vaga:",
    item_placeholder_local: "Local da vaga",
    item_msg_sem_local: "Local não informado",
    item_label_descricao: "Descrição:",
    item_placeholder_descricao: "Descrição da vaga",
    item_label_modalidade: "Modalidade de trabalho:",
    item_label_hibrido: "Híbrido",
    item_label_home: "Home Office",
    item_label_presenial: "Presencial",
    item_msg_sem_modalidade: "Modalidade não informada",
    item_label_periodo: "Período de trabalho:",
    item_label_integral: "Integral",
    item_label_meio: "Meio Período",
    item_label_hora: "Por Hora",
    item_msg_sem_periodo: "Período não informado",
    item_label_dias_vaga: "Quantos dias a vaga ficará disponível?",
    item_label_qtde_vaga: "Quantidade de vagas para este cargo:",
    item_label_vaga_afirmativa: "Vaga Afirmativa / Inclusiva:",
    item_vaga_pcd: "Vaga para PCD",
    item_vaga_lgbtq: "Vaga para LGBTQ+",
    item_vaga_mulheres: "Vaga para Mulheres",
    item_vaga_50_mais: "Vaga para 50+",
    item_msg_item_mulher: "Mulheres",
    item_msg_sem_foto: "Sem logo",
    item_botao_voltar: "Voltar",
    item_botao_cancelar: "Cancelar",
    item_botao_avancar: "Avançar",
    item_botao_publicar: "Publicar",
    item_msg_campo_obt: "Campo Obrigatório",
    item_alerta_descartada: "Alterações descartadas.",
    item_alerta_sucesso_1: "Vaga ",
    item_alerta_sucesso_2: "publicada com sucesso!",
    item_alerta_erro_buscar_dados: "Erro ao buscar dados da vaga",
    item_alerta_erro_salvar: "Erro ao salvar vaga.",
    item_botao_cadastrar: "+ Cadastrar Vaga",
    item_botao_editar: "Editar Vaga",
    item_botao_adicionar: "+ Adicionar",
    item_item_vagas: "Vagas",
    item_item_periodo: "Aberta até:",
    item_msg_vigencia: "Vigência até: ",
    item_msg_aberta: "Aberta em: ",
    item_msg_sem_data: "Data não informada",
    item_label_informe_hardskills:
      "Informe as Hard Skills que você busca para este perfil profissional",
    item_label_informe_hardskills_subitem:
      "Hard Skill são as competência técnicas adquiridas pelo profissional ao longo da sua carreira.",
    item_label_informe_softskills:
      "Informe as Soft Skills que você busca para este perfil profissional",
    item_label_informe_softskills_subitem:
      "Soft Skills são as competência comportamentais naturais ou adquiridas pelo profissional ao longo da sua carreira.",
    item_label_informe_qtde: "(mínimo 3 e no máximo 12)",
    item_label_skill: "Skills:",
    item_msg_skill: "Digite ou selecione uma skill",
    item_msg_criar_skill: "Criar nova skill:",
    item_label_skill_pesos: "Skills e Nível",
    item_label_peso: "Nível",
    item_label_avaliador: "Avaliador",
    item_label_proprio: "Próprio",
    item_label_whizzat: "Whizzat",
    item_botao_remover_skill: "Remover Skill",
    item_tooltip_skill_titulo: "Como adicionar skill que não está na lista:",
    item_tooltip_skill_passo1: "1. Digite a skill desejada;",
    item_tooltip_skill_passo2: "2. Selecione 'Criar nova skill';",
    item_tooltip_skill_passo3: "3. Clique no botão Adicionar.",
    item_tooltip_avaliador_passo1: "1. Próprio: Será selecionado ",
    item_tooltip_avaliador_passo2: "somente entre os avaliadores existentes ",
    item_tooltip_avaliador_passo3: "na empresa recrutadora.",
    item_tooltip_avaliador_passo4: "2. Whizzat: Será selecionado ",
    item_tooltip_avaliador_passo5: "automaticamente pela plataforma, ",
    item_tooltip_avaliador_passo6: "considerando melhores skills ",
    item_tooltip_avaliador_passo7: "e avaliações. ",
    item_tooltip_avaliador_passo8: "1. Whizzat: Será selecionado ",
    item_tooltip_avaliador_passo9: "automaticamente pela plataforma, ",
    item_tooltip_avaliador_passo10: "considerando melhores skills ",
    item_tooltip_avaliador_passo11: "e avaliações e quando a empresa ",
    item_tooltip_avaliador_passo12: "selecionada não possuir avaliador ",
    item_tooltip_avaliador_passo13: "próprio.",
    item_botao_lista_vagas: "← Voltar para lista de vagas",
    item_label_panel_score: "Score Médio (Hard+Soft):",
    item_label_panel_hardskill: "Distribuição de Hard Skills",
    item_label_panel_softskill: "Distribuição de Soft Skills",
    tipo_grafico_perfil: "Meu Score",
    tipo_grafico_avaliador: "Sugestão do avaliador",
    tipo_oportunidade_titulo: "Tipo de oportunidade",
    tipo_oportunidade_descricao:
      "Defina se a vaga é de ampla concorrência ou possui uma política afirmativa específica.",
    tipo_ampla: "Ampla concorrência",
    tipo_ampla_msg: "Oportunidade aberta a todos os candidatos compatíveis.",
    tipo_afirmativa: "Afirmativa",
    tipo_afirmativa_msg:
      "Oportunidade com foco ou priorização para públicos específicos.",
    tipo_exclusiva: "Exclusiva",
    tipo_exclusiva_msg:
      "Oportunidade destinada exclusivamente aos públicos selecionados.",
    publico_afirmativo_titulo: "Público da oportunidade",
    publico_afirmativo_msg:
      "Selecione os públicos contemplados pela ação afirmativa.",
    publico_exclusivo_msg:
      "Selecione os públicos aos quais esta oportunidade é destinada.",
    publico_pcd: "Pessoas com deficiência (PCD)",
    publico_racial: "Ações afirmativas étnico-raciais",
    publico_lgbtqia: "Diversidade LGBTQIA+",
    publico_mulheres: "Mulheres",
    publico_50mais: "Profissionais 50+",
    publico_diversidade: "Outras iniciativas de diversidade",
    publico_obrigatorio:
      "Selecione pelo menos um público para esta oportunidade.",
    tipo_oportunidade_detalhe_msg:
      "Classificação e público desta oportunidade.",
    publico_pcd_curto: "PCD",
    publico_racial_curto: "Étnico-racial",
    publico_lgbtqia_curto: "LGBTQIA+",
    publico_mulheres_curto: "Mulheres",
    publico_50mais_curto: "50+",
    publico_diversidade_curto: "Diversidade",
  },
  vaga_match: {
    titulo: "Encontrar candidatos",
    descricao:
      "Encontre os candidatos com maior compatibilidade com os critérios desta vaga.",
    compatibilidade: "Compatibilidade",
    compatibilidade_alta: "Alta compatibilidade",
    compatibilidade_boa: "Boa compatibilidade",
    compatibilidade_compativel: "Compatíveis",
    compatibilidade_todos: "Todos",
    quantidade: "Resultados",
    buscar: "Buscar candidatos",
    buscando: "Buscando...",
    criterios_considerados:
      "Critérios considerados automaticamente nesta busca:",
    criterio_skills: "Skills da vaga",
    criterio_modalidade: "Modalidade",
    criterio_localizacao: "Localização",
    criterio_oportunidade: "Critérios da oportunidade",
    resultados_titulo: "Candidatos compatíveis",
    resultados_descricao:
      "Resultados ordenados pela maior compatibilidade com esta vaga.",
    candidatos_encontrados: "candidatos encontrados",
    match_geral: "Match geral",
    hard_skills: "Hard Skills",
    soft_skills: "Soft Skills",
    modalidade_compativel: "Modalidade compatível",
    oportunidade_compativel: "Oportunidade compatível",
    skills_avaliadas: "skills avaliadas",
    ver_perfil: "Ver perfil",
    selecionados: "selecionado(s)",
    selecionar_todos: "Selecionar todos",
    convidar_selecionados: "Convidar selecionados",
    mais_acoes: "Mais ações",
    nao_mostrar_novamente: "Não mostrar novamente",
    ignorar_titulo: "Não mostrar este candidato novamente?",
    ignorar_descricao:
      "Este candidato deixará de aparecer nas suas próximas buscas. Você poderá reverter esta decisão futuramente.",
    ignorar_motivo: "Motivo",
    ignorar_motivo_opcional: "Selecione um motivo (opcional)",
    motivo_perfil: "Perfil não aderente",
    motivo_entrevista: "Histórico de entrevista",
    motivo_avaliado: "Já avaliado anteriormente",
    motivo_outro: "Outro",
    cancelar: "Cancelar",
    confirmar_ignorar: "Não mostrar novamente",
    sem_resultados: "Nenhum candidato disponível nesta busca.",
    limite_convites: "Você pode selecionar no máximo 10 candidatos por vez.",
    compatibilidade_alta_descricao: "Match mínimo: 90%",
    compatibilidade_boa_descricao: "Match mínimo: 75%",
    compatibilidade_compativel_descricao: "Match mínimo: 60%",
    compatibilidade_todos_descricao: "Sem match mínimo",
    ir_para_busca: "Encontrar candidatos",
    erro_buscar: "Não foi possível buscar candidatos para esta vaga.",
    publico_prioritario: "Público prioritário",
    ignorando: "Removendo...",
    ignorar_sucesso: "Candidato removido das próximas buscas.",
    erro_ignorar:
      "Não foi possível remover este candidato das próximas buscas.",
    menu_candidatos_ignorados: "Candidatos ignorados",
    enviando_convites: "Enviando convites...",
    convites_enviados_sucesso: "{{total}} convite(s) enviado(s) com sucesso.",
    erro_enviar_convite: "Não foi possível enviar os convites.",
    todos_ja_convidados:
      "Os candidatos selecionados já foram convidados para esta vaga.",
    convites_enviados_parcial:
      "{{total}} convite(s) enviado(s). {{jaConvidados}} candidato(s) já havia(m) sido convidado(s).",
    confirmar_convite_titulo: "Enviar convite?",
    confirmar_convite_descricao: "Confirme o envio antes de continuar.",
    confirmar_convite_um:
      "Você está prestes a convidar 1 candidato para esta vaga.",
    confirmar_convite_varios:
      "Você está prestes a convidar {{total}} candidatos para esta vaga.",
    confirmar_convite_aviso:
      "Após o envio, os candidatos serão notificados e poderão aceitar ou recusar o convite.",
    confirmar_convite_cancelar: "Cancelar",
    confirmar_convite_enviar: "Enviar convite",
    confirmar_convite_fechar: "Fechar",
    ja_convidado: "Já convidado",
    ja_convidado_anteriormente: "Já recebeu convite seu",
  },
  candidatos_ignorados: {
    titulo: "Candidatos ignorados",
    descricao: "Gerencie candidatos que você escolheu não mostrar nas buscas.",
    selecionar_todos: "Selecionar todos",
    selecionados: "selecionado(s)",
    restaurar_selecionados: "Restaurar selecionados",
    voltar_mostrar: "Voltar a mostrar",
    restaurando: "Restaurando...",
    sem_candidatos: "Você não possui candidatos ignorados.",
    sem_motivo: "Motivo não informado",
    motivo_perfil: "Perfil não aderente",
    motivo_entrevista: "Histórico de entrevista",
    motivo_avaliado: "Já avaliado anteriormente",
    motivo_outro: "Outro",
    erro_carregar: "Não foi possível carregar os candidatos ignorados.",
    erro_restaurar: "Não foi possível restaurar o candidato.",
    restaurar_sucesso: "Candidato voltou a aparecer nas suas buscas.",
    restaurar_varios_sucesso: "Candidatos restaurados com sucesso.",
  },
  planos: {
    titulo: "Você ainda não possui um plano, selecione abaixo.",
    recomendado: "Recomendado",
    botao_selecionar: "Selecionar Plano",
    botao_selecionado: "Plano Selecionado",
    mensal: "Mensal",
    anual: "Anual",
    mes: "mês",
    ano: "ano",
  },
  notificacao: {
    novo: "Novo",
    candidato: "Candidato:",
    enviado: "Enviado",
    marcar: "Marcar como lida",
    lida: "Lido",
    marcar_todos: "Marcar todas como lidas",
    filtro_todas: "Todas",
    filtro_nao_lidas: "Não lidas",
    titulo_confirma_exclusao: "Confirmar exclusão",
    msg_confirma_exclusao: "Deseja realmente excluir esta notificação?",
    botao_cancelar: "Cancelar",
    botao_excluir: "Excluir",
    msg_exclusao: "Notificação excluída com sucesso",
    msg_erro: "Erro ao excluir notificação",
    carregando: "Carregando mais...",
  },
  minha_avaliacao: {
    recebido: "Convites Recebidos",
    aceito: "Aceito/Ag. Questionário",
    agendado: "Agendar/Agendados",
    sem_convite: "Nenhum convite pendente",
    sem_aceito: "Nenhum convite aceito",
    sem_agenda: "Nenhum agendamento pendente",
    aguardando_finalizacao: "Aguardando finalização",
    aguardando_avaliacao: "Aguardando avaliação",
    aguardando_agenda: "Aguardando agendamento",
    aguardando_questionario: "Questionário Pendente",
    aguardando_entrevista: "Aguardando Entrevista",
    questionario_respondido: "Questionário Respondido",
    agenda_recusada: "Data Recusada",
    agenda_aceita: "Agenda Confirmada",
    btn_aceitar: "Aceitar",
    btn_recusar: "Recusar",
    peso: "Nível",
    aceito_em: "Aceito em ",
    enviado_em: "Enviado em ",
    sem_finalizacao: "Nenhuma avaliação finalizada",
    finalizado: "Finalizados",
    basico: "Básico",
    intermediario: "Intermediário",
    avancado: "Avançado",
    autoavaliacao: "Auto avaliação",
    ultima_avaliacao: "Última avaliação",
    dt_ultima_avaliacao: "Dt. Últ. Avaliação",
    titulo_progresso: "Progresso da Avaliação",
    etapa_atual: "etapa atual",
    titulo_questionario: "Selecionar Questionário",
    carregando_questionario: "Carregando questionários...",
    selecione_questionario: "Selecione um questionário",
    nao_questionario: "Não enviar questionário",
    enviar_questionario: "Enviar Questionário",
    enviando: "Enviando...",
    enviado_questionario: "Questionário enviado com sucesso",
    questionario: "Questionário",
    data_envio: "Data Envio",
    data_resposta: "Data Resposta",
    data_entrevista: "Data Entrevista",
    data_sugestao: "Sugestão enviada",
    horarios: "Horários",
    sugestao_selecionada: "Sugestão selecionada",
    nova_sugestao: "Enviar Nova Sugestão",
    enviada_sugestao: "Sugestão enviada",
    enviar_sugestao: "Enviar Sugestão",
    confirmar_entrevista: "Confirmar entrevista realizada",
    msg_agenda:
      "Disponível após envio do questionário ou caso o avaliador opte por não enviar",
    avaliacao_finalizada: "Avaliação Finalizada!",
    erro_problema: "Ocorreu uma ação inesperada. Tente mais tarde.",
    finalizar_avaliacao: "Finalizar Avaliação",
    finalizada_avaliacao: "Avaliação Finalizada",
    informe_nota: "Informe a nota final e comentário da entrevista",
    nota_final: "Nota Final",
    comentario:
      "Comentário (visível apenas para você, este conteúdo não será exibido ao candidato)",
    comentario_resposta: "Comentário sobre respostas",
    finalizando: "Finalizando...",
    btn_ver_resposta: "Visualizar Repostas",
    btn_ocultar_resposta: "Ocultar Respostas",
    sem_questionario: "Nenhum questionário encontrado",
    msg_salvar_comentario: "Comentário salvo.",
    informe_comentario: "Necessário informar um comentário",
    respostas_questionario: "Respostas do Questionário",
    salvando: "Salvando...",
    salvar_comentario: "Salvar Comentário",
    comentario_finalizacao:
      "Comentário (este conteúdo será exibido ao candidato como feedback. Utilize uma linguagem clara, objetiva e construtiva.)",
    steps: {
      convite_aceito: "CONVITE ACEITO",
      questionario_enviado: "QUESTIONÁRIO ENVIADO",
      agenda_enviada: "AGENDA ENVIADA",
      agendado: "AGENDADO",
      entrevista_realizada: "ENTREVISTA REALIZADA",
      finalizado: "FINALIZADO",
    },
    status_agenda: {
      pendente: "Aguardando Aceite",
      aceito: "Aceita",
      recusado: "Recusada",
    },
    candidato: {
      questionarios_pendentes: "Questionário Pendente",
      entrevistas: "Entrevista Sugerida",
      finalizadas: "Finalizadas",
      sem_questionario: "Nenhum questionário pendente",
      sem_entrevista: "Nenhuma entrevista agendada",
      sem_finalizacao: "Nenhuma avaliação finalizada",
      avaliacao_finalizada: "Avaliação Finalizada",
      atualizado_em: "Atualizado em: ",
      entrevista_confirmada: "Entrevista Confirmada",
      responder_questionario: "Aguardando Resposta",
      btn_recusar: "Recusar",
      btn_aceitar: "Aceitar",
      peso: "Peso:",
      entrevista_em: "Entrevista",
      questionario_pendente: "Aguardando Resposta",
      formulario_enviado: "Questionário Recebido:",
      entrevista_recusada: "Data Recusada, aguardar nova data.",
      resultado_avaliacao: "Resultado da Avaliação",
      resultado_descricao:
        "Confira abaixo sua autoavaliação, a nota atribuída pelo avaliador e o nível identificado durante o processo.",
      basico: "Básico",
      intermediario: "Intermediário",
      avancado: "Avançado",
      comentario: "Feedback da avaliação",
      comentario_msg:
        "Abaixo estão os comentários compartilhados pelo avaliador sobre sua entrevista.",
    },
  },
  questionario: {
    titulo_pagina: "Criação de Questionário",
    pagina_descricao: "",
    titulo: "Título do Questionário",
    place_titulo: "Informe o título para seu questionário",
    comentario: "Comentário",
    place_comentario:
      "Coloque se desejar um comentário para descrever para que serve ou se destina esse questionário",
    ativo: "Ativo",
    inativo: "Inativo",
    pergunta: "Perguntas",
    btn_add_pergunta: "Adicionar Pergunta",
    sem_pergunta: "Ainda não há perguntas em seu questionário",
    btn_cancelar: "Cancelar",
    btn_salvar: "Salvar",
    btn_salvando: "Salvando...",
    criado_em: "Criado em",
    titulo_pagina_questionario: "Lista de Questionários",
    btn_add_questionario: "Novo Questionário",
    btn_ver: "Ver",
    btn_editar: "Editar",
    btn_excluir: "Excluir",
    sem_questionario: "Nenhum questionário criado ainda",
    sucesso: "Solicitação executada com sucesso!",
    erro: "Problemas ao tentar executar a operação!",
    nao_encontrado: "Questionário não encontrado",
    ver_pergunta: "Pergunta",
    btn_voltar: "Voltar",
    resposta_base: "Resposta base",
    tipo_resposta: "Tipo Resposta",
    titulo_editar: "Edição de Questionário",
    btn_excluindo: "Excluindo..",
    filtro: "Filtro",
    modal_titulo: "Excluir Questionário",
    modal_pergunta: "Deseja realmente excluir",
    anterior: "Anterior",
    proximo: "Próximo",
    pagina_count: "Página",
    de_count: "de",
    msg_titulo_obg: "Informe o título do questionário",
    msg_pergunta_obg: "Adicione pelo menos uma pergunta",
    msg_todaspergunta_obg: "Todas as perguntas devem ser preenchidas",
    excluir_pergunta: "Excluir Pergunta",
    duplicar_pergunta: "Duplicar Pergunta",
    msg_exclusao_pergunta:
      "Caso a pergunta já foi enviada a um candidato, a mesma será apenas inativada.",
    msg_nenhuma_pergunta: "Nenhuma pergunta adicionada ainda",
    ativo_sim: "Sim",
    ativo_nao: "Não",
    resultado_busca: "questionário(s) encontrado(s)",
    filtro_busca: "Buscar questionário...",
    filtro_ativo_todos: "Todos",
    filtro_ativo: "Ativo",
    filtro_inativo: "Inativo",
    obrigatorio: "Obrigatório",
    resposta: {
      titulo: "Questionário de Avaliação",
      titulo_skill: "Skill avaliada:",
      titulo_respondida: "Respondidas:",
      titulo_de: "de",
      btn_cancelar: "Cancelar",
      salvando: "Salvando...",
      btn_salvar: "Enviar Respostas",
      msg_sem_questionario: "Questionário não encontrado.",
      msg_carregando: "Carregando questionário...",
      msg_erro: "Erro ao salvar questionário",
      msg_sucesso: "Questionário enviado com sucesso",
      msg_preencher_campo:
        "Preencha todas as perguntas obrigatórias para enviar o questionário.",
      finalizar_sem_entrevista: "Permitir Finalizar Sem Entrevista.",
    },
  },
  agenda: {
    titulo_um: "Agenda de Entrevistas",
    titulo_dois: "Consulte abaixo suas próximas entrevistas agendadas.",
    status_aguardando: "Aguardando aceite",
    status_agendada_atrasada: "Agendada / Em atraso",
    status_agendada: "Agendada",
    status_atrasada: "Em atraso",
    sem_entrevista: "Nenhuma entrevista agendada",
    msg_sem_entrevista:
      "Quando existir uma entrevista aceita ela aparecerá aqui.",
    proxima_entrevista: "Próximas entrevistas",
    hoje: "Hoje",
    amanha: "Amanhã",
    entrevista_agendada: "Entrevistas agendadas",
    entrevista_pendente: "Entrevistas pendentes/atraso",
    domingo: "Dom",
    segunda: "Seg",
    terca: "Ter",
    quarta: "Qua",
    quinta: "Qui",
    sexta: "Sex",
    sabado: "Sáb",
    sem: "sem",
  },
  dash_candidato: {
    header: "Meu Dashboard",
    sub_header:
      "Acompanhe seus processos seletivos, entrevistas e evolução do seu perfil.",
    skill_header: "Meu perfil",
    skill_titulo: "Compare seu nível informado com a avaliação recebida.",
    skill_hard: "Hard Skills",
    skill_soft: "Soft Skills",
    skill_perfil: "Meu Score",
    skill_avaliador: "Avaliador",
    skill_hard_descricao: "Competências técnicas",
    skill_soft_descricao: "Competências comportamentais",
    sem_avaliacao: "Avaliação ainda não disponível",
    erro: "Não foi possível carregar o dashboard.",
    stats_seletivo: "Processos seletivos",
    stats_seletivo_msg: "Em andamento",
    stats_agendada: "Compromissos agendados",
    stats_agendada_msg: "Confirmados",
    stats_realizada: "Entrevistas realizadas",
    stats_realizada_msg: "Concluídas",
    stats_skill: "Skills avaliadas",
    stats_skill_msg: "Com avaliação",
    avaliacao_confirmada: "Avaliações confirmadas com seus avaliadores.",
    sem_agenda: "Você não possui entrevistas agendadas.",
    agenda_skill: "Avaliação de Skill",
    agenda_atrasada: "Atrasada",
    agenda_confirmada: "Confirmada",
    agenda_processo: "Processo seletivo",
    agenda_vaga: "Processo seletivo",
    agenda_palestra_evento: "Palestra / Evento",
    agenda_mentoria: "Mentoria",
    agenda_projeto_consultoria: "Projeto / Consultoria",
    agenda_networking: "Networking",
    agenda_outro: "Outro",
    agenda_descricao: "Seus próximos compromissos confirmados.",
    stats_oportunidades: "Outras oportunidades",
    stats_oportunidades_msg: "Em andamento",
    oportunidades_titulo: "Minhas oportunidades",
    oportunidades_descricao: "Acompanhe suas oportunidades em andamento.",
    sem_oportunidades: "Você não possui oportunidades em andamento.",
    ver_todas_oportunidades: "Ver todas",
    ver_processo: "Ver processo",
    ver_oportunidade: "Ver oportunidade",
    compatibilidade: "Compatibilidade",
    ultima_atualizacao: "Última atualização",

    tipo_vaga: "Processo seletivo",
    tipo_palestra_evento: "Palestra / Evento",
    tipo_mentoria: "Mentoria",
    tipo_projeto_consultoria: "Projeto / Consultoria",
    tipo_networking: "Networking",
    tipo_outro: "Outro",

    status_convite_aceito: "Convite aceito",
    status_agenda_enviada: "Aguardando confirmação",
    status_agendado: "Entrevista agendada",
    status_entrevista_realizada: "Entrevista realizada",
    oportunidade_agendada: "Entrevista confirmada",
    movimentacoes_titulo: "Movimentações recentes",
    movimentacoes_descricao:
      "Acompanhe as últimas movimentações das suas oportunidades.",
    sem_movimentacoes: "Nenhuma movimentação recente",
    sem_movimentacoes_msg: "Suas próximas movimentações aparecerão aqui.",
    movimentacao_convite_recebido: "Convite recebido",
    movimentacao_convite_aceito: "Convite aceito",
    movimentacao_agenda_enviada: "Novo horário recebido",
    movimentacao_agenda_confirmada: "Horário confirmado",
    movimentacao_processo_finalizado: "Processo finalizado",
  },
  dash_avaliador: {
    stats_pendente: "Avaliações pendentes",
    stats_pendente_msg: "Aguardando conclusão",
    stats_agendada: "Entrevistas agendadas",
    stats_agendada_msg: "Confirmadas",
    stats_skill: "Skills avaliadas",
    stats_skill_msg: "Total realizado",
    stats_pontuacao: "Pontuação",
    stats_pontuacao_msg: "Pontuação atual",
    header: "Meu Dashboard",
    sub_header:
      "Acompanhe suas avaliações, entrevistas e desempenho como avaliador.",
    erro: "Não foi possível carregar o dashboard.",
    agenda_titulo: "Entrevistas agendadas",
    agenda_descricao: "Avaliações confirmadas com candidatos.",
    agenda_atrasada: "Atrasada",
    agenda_confirmada: "Confirmada",
    sem_agenda: "Você não possui entrevistas agendadas.",
    pendencias_titulo: "Avaliações que exigem ação",
    pendencias_descricao: "Avaliações que aguardam alguma ação sua.",
    acao_questionario: "Enviar questionário",
    acao_agenda: "Enviar agenda",
    acao_reagendar: "Reagendar entrevista",
    acao_finalizar: "Finalizar avaliação",
    btn_ver_avaliacao: "Ver avaliação",
    sem_pendencias: "Nenhuma avaliação exige sua atenção no momento.",
    top_skills_titulo: "Skills mais avaliadas",
    top_skills_descricao: "Competências que você mais avaliou.",
    sem_top_skills: "Você ainda não possui avaliações finalizadas.",
    ranking_titulo: "Pontuação e ranking",
    ranking_descricao:
      "Sua pontuação influencia a prioridade no recebimento de avaliações.",
    ranking_pontuacao: "Pontuação atual",
    ranking_pontos: "pontos",
    ranking_posicao: "Posição no ranking",
    ranking_avaliadores: "avaliadores",
  },
  perfil_candidato: {
    sobre: "Sobre",
    modalidades: "Modalidades de trabalho",
    hard_skills: "Hard Skills",
    soft_skills: "Soft Skills",
    formacao: "Formação acadêmica",
    certificacoes: "Certificações",
    avaliado: "Avaliado",
    autoavaliacao: "Autoavaliação",
    sem_apresentacao: "O candidato não informou uma apresentação.",
    sem_modalidades: "Nenhuma modalidade informada.",
    sem_skills: "Nenhuma skill informada.",
    sem_formacao: "Nenhuma formação acadêmica informada.",
    sem_certificacoes: "Nenhuma certificação informada.",
    fechar: "Fechar",
    selecionar: "Selecionar candidato",
    remover_selecao: "Remover seleção",
    ver_perfil: "Ver perfil",
    erro_carregar: "Não foi possível carregar o perfil do candidato.",
  },
  buscar_candidatos: {
    convite_titulo: "Convidar candidatos",
    convite_descricao:
      "Envie um convite profissional aos candidatos selecionados.",

    convite_candidato_selecionado: "candidato selecionado",
    convite_candidatos_selecionados: "candidatos selecionados",
    convite_mesma_mensagem: "Todos receberão este mesmo convite.",

    convite_motivo: "Motivo do convite",
    convite_motivo_descricao: "Selecione o contexto do contato.",

    convite_tipo_oportunidade: "Oportunidade profissional",
    convite_tipo_oportunidade_descricao:
      "Convite para uma oportunidade profissional.",

    convite_tipo_palestra: "Palestra / evento",
    convite_tipo_palestra_descricao:
      "Participação em palestra, evento ou encontro.",

    convite_tipo_mentoria: "Mentoria",
    convite_tipo_mentoria_descricao:
      "Convite relacionado a mentoria ou orientação.",

    convite_tipo_projeto: "Projeto / consultoria",
    convite_tipo_projeto_descricao:
      "Participação em projeto ou trabalho de consultoria.",

    convite_tipo_networking: "Networking / conversa",
    convite_tipo_networking_descricao:
      "Contato profissional ou conversa inicial.",

    convite_tipo_outro: "Outro",
    convite_tipo_outro_descricao: "Outro motivo de contato profissional.",

    convite_campo_titulo: "Título",
    convite_titulo_placeholder:
      "Ex.: Convite para palestra sobre gestão de projetos",

    convite_campo_mensagem: "Mensagem",
    convite_mensagem_placeholder:
      "Conte um pouco sobre o convite, contexto e por que gostaria de entrar em contato.",
    convite_mensagem_alerta:
      "Evite incluir informações sensíveis ou confidenciais.",

    convite_btn_cancelar: "Cancelar",
    convite_btn_enviar: "Enviar convite",

    convite_mock_sucesso_um: "Convite preparado para 1 candidato.",
    convite_mock_sucesso_varios: "Convite preparado para {{total}} candidatos.",
    convite_btn_fechar: "Fechar",
    convite_conflito_titulo:
      "{{quantidade}} candidato(s) já possui(em) um convite ativo deste tipo",
    convite_conflito_descricao:
      "Esses candidatos não receberão um novo convite enquanto o processo atual estiver em andamento.",
    convite_ativo: "Convite ativo",
    convite_btn_enviar_quantidade: "Enviar para {{quantidade}} candidato(s)",
    convite_enviando: "Enviando...",
  },
  oportunidades_candidato: {
    titulo: "Minhas oportunidades",
    descricao: "Acompanhe seus convites, entrevistas e processos finalizados.",
    convites: "Convites",
    entrevistas: "Entrevistas",
    finalizados: "Finalizados",
    recebido_em: "Recebido em",
    finalizado_em: "Finalizado em",
    btn_aceitar: "Aceitar convite",
    btn_recusar: "Recusar",
    btn_aceitar_horario: "Aceitar horário",
    btn_recusar_horario: "Recusar horário",
    aguardando_resposta_agenda: "Aguardando sua confirmação",
    aprovado: "Aprovado",
    nao_aprovado: "Não aprovado",
    parecer: "Feedback do recrutador",
    tipo_vaga: "Vaga",
    tipo_oportunidade: "Oportunidade",
    tipo_evento: "Palestra / Evento",
    tipo_mentoria: "Mentoria",
    tipo_projeto: "Projeto / Consultoria",
    tipo_networking: "Networking",
    tipo_outro: "Outro",
    carregando_convites: "Carregando seus convites...",
    erro_convites: "Não foi possível carregar seus convites.",
    sem_convites: "Nenhum convite pendente",
    sem_convites_msg:
      "Quando você receber um novo convite, ele aparecerá aqui.",
    horario_recusado: "Horário recusado",
    aguardando_nova_agenda:
      "Aguardando uma nova sugestão de horário do recrutador.",
    aguardando_agenda: "Aguardando o recrutador sugerir um horário.",
    entrevista_confirmada: "Entrevista confirmada",
    entrevista_realizada: "Entrevista realizada",
    aguardando_finalizacao:
      "Entrevista concluída. Aguardando a finalização do processo pelo recrutador.",
    carregando_finalizados: "Carregando processos finalizados...",
    erro_finalizados: "Não foi possível carregar os processos finalizados.",
    sem_finalizados: "Nenhum processo finalizado",
    sem_finalizados_msg: "Seus processos concluídos aparecerão aqui.",
    processo_finalizado: "Processo finalizado",
    carregando_entrevistas: "Carregando suas entrevistas...",
    erro_entrevistas: "Não foi possível carregar suas entrevistas.",
    sem_entrevistas: "Nenhuma entrevista em andamento",
    sem_entrevistas_msg:
      "Quando você aceitar um convite, o andamento do processo aparecerá aqui.",
  },
  processos_recrutador: {
    titulo: "Processos",
    descricao:
      "Gerencie convites, entrevistas e processos dos seus candidatos.",

    em_andamento: "Em andamento",
    finalizados: "Finalizados",
    recusados: "Recusados",

    tipo_vaga: "Vaga",
    tipo_oportunidade: "Oportunidade",
    tipo_evento: "Palestra / Evento",
    tipo_mentoria: "Mentoria",
    tipo_projeto: "Projeto / Consultoria",
    tipo_networking: "Networking",
    tipo_outro: "Outro",

    convite_aceito: "Convite aceito",
    convite_recusado: "Convite recusado",

    sugerir_entrevista_msg:
      "O candidato aceitou o convite. Sugira uma data e horário para a entrevista.",
    horario_recusado: "Horário recusado",
    sugerir_novo_horario_msg:
      "O candidato recusou o horário sugerido. Envie uma nova opção.",

    aguardando_confirmacao_candidato: "Aguardando confirmação do candidato",
    entrevista_confirmada: "Entrevista confirmada",
    entrevista_realizada: "Entrevista realizada",
    finalizar_processo_msg:
      "A entrevista foi realizada. O processo já pode ser finalizado.",

    btn_sugerir_entrevista: "Sugerir entrevista",
    btn_novo_horario: "Sugerir novo horário",
    btn_realizada: "Marcar como realizada",
    btn_finalizar: "Finalizar processo",

    aprovado: "Aprovado",
    nao_aprovado: "Não aprovado",
    parecer: "Feedback para o candidato",

    finalizado_em: "Finalizado em",
    recusado_em: "Recusado em",
    modal_agenda: {
      titulo: "Sugerir entrevista",
      titulo_novo: "Sugerir novo horário",
      descricao:
        "Escolha uma data e horário para a entrevista com o candidato.",
      data: "Data da entrevista",
      hora: "Horário",
      aviso:
        "O candidato receberá a sugestão e poderá aceitar ou recusar o horário.",
      horario_anterior_recusado: "Horário anterior recusado pelo candidato",
      btn_cancelar: "Cancelar",
      btn_enviar: "Enviar sugestão",
      btn_novo: "Enviar novo horário",
      data_invalida: "Selecione uma data e horário futuros.",
      btn_enviando: "Enviando...",
      erro: "Não foi possível enviar a sugestão de entrevista.",
    },
    sucesso_marcar_realizada: "Entrevista marcada como realizada com sucesso.",
    erro_marcar_realizada:
      "Não foi possível marcar a entrevista como realizada.",
    btn_marcando_realizada: "Marcando...",
    modal_finalizacao: {
      titulo: "Finalizar processo",
      descricao:
        "Informe o resultado da entrevista e compartilhe um feedback com o candidato.",
      resultado: "Resultado do processo",
      aprovado: "Aprovado",
      nao_aprovado: "Não aprovado",
      feedback: "Feedback para o candidato",
      feedback_placeholder:
        "Escreva um feedback sobre a participação do candidato no processo...",
      feedback_aviso: "Este feedback será exibido ao candidato.",
      btn_cancelar: "Cancelar",
      btn_finalizar: "Finalizar processo",
      btn_finalizando: "Finalizando...",
      sucesso: "Processo finalizado com sucesso.",
      erro: "Não foi possível finalizar o processo.",
    },
    convites: "Convites",
    carregando_convites: "Carregando convites...",
    sem_convites: "Nenhum convite aguardando resposta",
    sem_convites_descricao:
      "Os convites enviados que ainda aguardam resposta do candidato aparecerão aqui.",
    aguardando_resposta: "Aguardando resposta",
    enviado_em: "Enviado em",
    erro_carregar_convites: "Não foi possível carregar os convites.",
  },
  dash_recrutador: {
    header: "Meu Dashboard",
    sub_header:
      "Acompanhe seus convites, entrevistas e processos de recrutamento.",
    stats_convites: "Convites enviados",
    stats_convites_msg: "Aguardando resposta",
    stats_andamento: "Processos em andamento",
    stats_andamento_msg: "Processos ativos",
    stats_agendada: "Entrevistas agendadas",
    stats_agendada_msg: "Confirmadas",
    stats_finalizados: "Processos finalizados",
    stats_finalizados_msg: "Total concluído",
    agenda_titulo: "Entrevistas agendadas",
    agenda_descricao: "Próximas entrevistas confirmadas com candidatos.",
    agenda_confirmada: "Confirmada",
    agenda_atrasada: "Atrasada",
    sem_agenda: "Nenhuma entrevista agendada.",
    pendencias_titulo: "Processos que exigem ação",
    pendencias_descricao: "Processos que aguardam uma ação sua para continuar.",
    acao_agenda: "Sugerir entrevista",
    acao_finalizar: "Finalizar processo",
    btn_ver_processo: "Ver processo",
    sem_pendencias: "Nenhum processo aguardando sua ação.",
    stats_vagas: "Vagas abertas",
    stats_vagas_msg: "Disponíveis para candidatos",
    erro: "Não foi possível carregar o dashboard.",
  },
};

export default pt;
